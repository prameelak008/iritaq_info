...
class Mailsms extends Admin_Controller
{
    ...
    // }
        ...
        $birthStudents                    =  $this->Entrance_settings_model->get_applicants($entrance_current_session['cur_session']);
        ...
        $array[] = array('name'     =>  $student_value['entrance_reg_name'],
        'contact_no'            =>  $student_value['entrance_reg_phone'],
        );
        }
        $birthDaysList['students'] = $array;
        }
        ...
        $this->load->view('entrance_settings/compose_whatsapp', $data);
...
*/
        ...
        $getusermobile      = "";
            ...
            $this->messages_model->add_mess($data);
            ...
            foreach ($userlisting as $users_key => $users_value) {
                if ($users_value == "student") {
                    $student_array = $this->Entrance_settings_model->get_applicants($entrance_current_session['cur_session']);

                    if (!empty($student_array)) {
                        foreach ($student_array as $student_key => $student_value)
                        {
                            $array = array(
                                'user_id'  => $student_value['entrance_reg_id'],
                                'email'    => $student_value['entrance_reg_email'],
                                'mobileno' => $student_value['entrance_reg_phone'],
               ...
               else if ($users_value == "parent")
               {
                    ...
                    $parent_array = $this->Entrance_settings_model->get_applicants($entrance_current_session['cur_session']);
                    if (!empty($parent_array)) {
                        foreach ($parent_array as $parent_key => $parent_value) {
                            $array = array(
                               'user_id'   => $parent_value['entrance_reg_id'],
                                'email'    => $parent_value['entrance_reg_email'],
                                'mobileno' => $parent_value['admission_phoneno'],
                            );
                            $user_array[] = $array;
                        }
                    }
           ...
           if (!empty($user_array)) 
            {
                ...
                foreach ($user_array as $user_mail_key => $user_mail_value)
                 {
                    if (in_array("push_whatsapp", $sms_mail)) 
                    {
                    if ($user_mail_value['mobileno'] != "") 
                    {
                    $numberlength = strlen((string) $user_mail_value['mobileno']);
                    if($numberlength<12)
                    {
                    $getusermobile='91'.''.$user_mail_value['mobileno'];
                    }
                    else
                    {
                    $getusermobile=$user_mail_value['mobileno'];
                    }      
                            
                    $this->smsgateway->sendWhatsAppSMS($getusermobile,$message );
                    }
                    }
                }
            }
...
/*
    ...
    public function timer_individual_whatsapp()
    {
