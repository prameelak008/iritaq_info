                    ...
                    class Sayexam extends Student_Controller
                    {
                    ...
                    $this->session->set_userdata('sub_menu', 'sayexam/sayexam');
                    ...
                    $data['sayexam_approval']    = $this->examgroupstudent_model->sayexam_approvedbyadmin($exam_group_id, $exam_id);
                    $data['examList']            = $this->examgroup_model->getExamByExamGroup($exam_group_id, true);
                    ...
                    $data['getstud']                  =   $this->examgroupstudent_model->getstud_sayexam($exam_group_id, $exam_id);
                    
                    $data['sayexam_marks']            =   $this->examresult_model->getsayexam_marks($exam_id, $exam_group_id,$student_id);
                             
                    $data['sayexamfee_payment']       =   $this->examgroup_model->sayexamfee_payment($exam_id,$exam_group_id,$student_id,$session_id);  
                    
                    $data['sayexamfee_feecharge_ce']  =   $this->examgroup_model->sayexamfee_feecharge_cemarks($exam_group_id,$exam_id);
                    
                    $this->load->view('layout/student/header',$data);
                    $this->load->view('user/sayexam/index', $data);
                    ...
                    public function sayexam_previous()
                    {
                    ...
                    $examGroupId              = $this->input->post('examGroupId');
                    $examId                   = $this->input->post('examId');
                    
                    
                    
                    $this->load->view('layout/student/header',$data);
                    $this->load->view('user/sayexam/previous_exam', $data);
                    $this->load->view('layout/student/footer',$data);
                    }
                    ...
                    echo json_encode($data);
                            ...
                            // $q = $this->db->get('exam_group_exam_sayexam');
                            // if ($q->num_rows() > 0) 
                            // {
                            ...
                            // $this->db->insert('exam_group_exam_sayexam', $dyt[$i]);
                            
                            // }
                            // }
                            // $data['sayexam_marks'] = $this->examresult_model->getsayexam_marks($exam_group_exam_results_exam_idd, $exam_group_exam_results_exam_groupidd,$student_id); 
                            // $this->load->view('layout/student/header',$data);
                            // $this->load->view('user/sayexam/sayexamview', $data);
                            // $this->load->view('layout/student/footer',$data);
                            ...
                            $existing = $this->db->get('exam_group_exam_sayexam')->result_array();
                            ...
                            for ($i = 0; $i < count($checked_result_ids); $i++) {
                                ...
                                if (isset($existing_map[$result_id])) {
                                    ...
                                    $this->db->where('exam_group_exam_sayexam_id', $existing_map[$result_id]['exam_group_exam_sayexam_id'])
                                             ->update('exam_group_exam_sayexam', $data);
                                } else {
                                    // New → Insert
                                    $this->db->insert('exam_group_exam_sayexam', $data);
                                }
                            }
                            ...
                            foreach ($existing_map as $result_id => $row) 
                            {
                                if (!in_array($result_id, $keep_ids)) 
                                {
                                    $this->db->where('exam_group_exam_sayexam_id', $row['exam_group_exam_sayexam_id'])
                                             ->delete('exam_group_exam_sayexam');
                                }
                            }
                            ...
                            $data['sayexam_marks']                          = $this->examresult_model->getsayexam_marks($exam_group_exam_results_exam_idd, $exam_group_exam_results_exam_groupidd,$student_id);  
                             ...
                             $data['Exam_group_list']   = $this->examgroup_model->getExamBy_NewExamGroup($group_id, $exam_id);
                            $this->load->view('layout/student/header',$data);
                            $this->load->view('user/sayexam/sayexamview', $data);
                            $this->load->view('layout/student/footer',$data);
                            ...
                            $data['exam_section_id']            = $this->input->post('exam_section_id');
                            $data['sayexam_marks']              = $this->examresult_model->getsayexam_marks($exam_group_exam_results_exam_idd, $exam_group_exam_results_exam_groupidd,$student_id);
                            $data['sayexamfee_feecharge_ce']    = $this->examgroup_model->sayexamfee_feecharge_cemarks($exam_group_exam_results_exam_groupidd,$exam_group_exam_results_exam_idd);
                            $this->load->view('layout/student/header',$data);
                            $this->load->view('user/sayexam/sayexam_proceedtopayment', $data);
                            $this->load->view('layout/student/footer',$data);
                            }                           
                            ...
                            $data['amount']          = $this->input->post('totalamt');
                            ...
                            $data['session_id']      = $this->input->post('session_id');
                            $this->load->view('layout/student/header', $data);
                            // $this->load->view('user/revaluation/standard/meTrnReq_sayexam', $data);
                            $this->load->view('revpayment_store/techprocess_ce',$data);
                            $this->load->view('layout/student/footer', $data);
                            }
                            ...
                            $data['parameters']      = file_get_contents(APPPATH . 'views/revpayment_store/parameters.json'); 
                            $this->load->view('layout/student/header', $data);
                            $this->load->view('revpayment_store/response_ce',$data);
                            $this->load->view('layout/student/footer',$data); 
                            }
                            ...
                            $data['paymentconfig']   = $this->user_model->payment_config();    
                            $this->load->view('user/revaluation/standard/meTrnSuccess_sayexam',$data);
                            }
                            ...
                            $this->load->view('layout/student/header', $data);
                            $this->load->view('user/revaluation/standard/meTrnPay_sayexam',$data);
                            $this->load->view('layout/student/footer', $data);
                            }  
                            ...
                            $this->db->insert("fees_sayexampayment", $feepay);
                            ...
                            $data['sayexam_marks']      = $this->examresult_model->getsayexam_marks($post_exam_id, $post_exam_group_id,$student_id);
                            $data['sch_setting']        = $this->sch_setting_detail;
                            
                            $data['sayexamfee_payment']    =  $this->examgroup_model->sayexamfee_payment($post_exam_id, $post_exam_group_id,$student_id,$session_id); 
                            $data['sayexamfee_feecharge_ce']    = $this->examgroup_model->sayexamfee_feecharge_cemarks($post_exam_group_id,$post_exam_id);
                            
                            $student_exam_page = $this->load->view('user/sayexam/_printsayexam', $data, true); 
