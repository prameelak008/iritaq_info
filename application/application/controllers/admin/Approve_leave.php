...
class approve_leave extends Admin_Controller {
    ...
    public function add() {
        ...
        $this->form_validation->set_rules('leave_from_time', $this->lang->line('from') . " " . $this->lang->line('time'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('leave_to_time', $this->lang->line('to') . " " . $this->lang->line('time'), 'trim|required|xss_clean');
    ...
    public function handle_upload($str,$var)
    {
        ...
        $leavid           =   $this->input->post('leavid');
        $reportedtime     =   $this->input->post('reportedtime');
        $reportingtime    =   $this->input->post('reportingtime');
        
        $perc             =   $this->input->post('perc');
         
         
        $report           =   $reported.' '.$reportedtime;
        $reportingval     =   date("Y-m-d", strtotime($reporting));
        $datetime_1       =   $reportingval.''.$reportingtime;
        $datetime_2       =   $report;
        
        
        
        $datetime1        =   strtotime( $datetime_1);
        $datetime2        =   strtotime($datetime_2);
        $diff             =   $datetime2-$datetime1;
        $hrs              =   floor($diff/3600);
        $remain           =   $diff - $hrs * 3600;        
        $value= sprintf('%02d',$hrs).gmdate(':i:s',$remain);
        ...
        $accuracy= sprintf('%02d:%02d', $hours, $minutes);
        $data 				        = 	array('reported_time' => $report,								
        'reported_remarks'     		=>	$remarks,
        'reporting_status'          =>  $perc,
        'reporting_accuracy'     	=>  $accuracy,
        'reporting_updated_time'    =>  date('Y-m-d H:i:s')
        );
        
        $condition 				    = 	array('id' => $leavid);
        $this->db->where($condition);
        $query=$this->db->update('student_applyleave',$data);
        ...
        echo json_encode($query);
            ...
            function delete_reportingtime()
            {
            $reported         =   $this->input->post('reported');
            $remarks          =   $this->input->post('remarks');
            $leavid           =   $this->input->post('leavid');
            $data 				        = 	array('reported_time' => " ",								
            'reported_remarks'     		=>	" ",
            'reporting_accuracy'     	=>  " ",
            'reporting_updated_time'    => date('Y-m-d H:i:s'));
            $condition 				    = 	array('id' => $leavid);
