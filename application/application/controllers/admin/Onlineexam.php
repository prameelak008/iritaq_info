                  ...
                  if($subject_value->is_quiz)
                   {
                    $is_quiz= "<i class='fa fa-check-square-o'    data-toggle='tooltip' title=' " .$this->lang->line('quiz_mode_enabled'). " ' ></i><span style='display:none'>Yes</span>" ;
                   }else{
                    $is_quiz= "<i class='fa fa-exclamation-circle'  data-toggle='tooltip' title=' " .$this->lang->line('quiz_mode_disabled'). " ' ></i><span style='display:none'>No</span>" ;
                   }
                ...
                if($subject_value->is_active == 1){ 
                        $is_active="<i class='fa fa-check-square-o' data-toggle='tooltip' title=' " .$this->lang->line('exam_publish_enabled'). " '></i><span style='display:none'>Yes</span>" ;
                    } else{
                        $is_active="<i class='fa fa-exclamation-circle' data-toggle='tooltip' title=' " .$this->lang->line('exam_publish_disabled'). " '></i><span style='display:none'>No</span>" ;
                    }   
                    
                    
                    if($subject_value->publish_result == 1){
                        $publish_result= "<i class='fa fa-check-square-o'  data-toggle='tooltip' title=' " .$this->lang->line('publish_result_enabled'). " ' ></i><span style='display:none'>Yes</span>" ;
                    }else{
                        $publish_result= "<i class='fa fa-exclamation-circle' data-toggle='tooltip' title=' " .$this->lang->line('publish_result_disabled'). " '></i><span style='display:none'>No</span>" ;
                    }
                    ...
                    {                       
                    $getStaffID               = $this->customlib->getStaffID();                    
                    
                    // if (!$this->rbac->hasPrivilege('onlineExamination', 'can_view')) 
                    // {
                    // access_denied();
                    // }

                    $data=array();
                    $this->session->set_userdata('top_menu', 'manage_application');
                    $this->session->set_userdata('sub_menu', 'manage_application/complete_exam');
                    
                    $data['classList']         = $this->class_model->get();
                    $data['subjectlist']       = $this->onlineexam_model->get_subjectlist($getStaffID);
                    
                    $data['class_by_teacher']  = $this->onlineexam_model->get_class_by_teacher($getStaffID);
                    
                    $this->load->view('layout/header', $data);
                    $this->load->view('admin/onlineexam/online_examination', $data);
                    $this->load->view('layout/footer', $data);
                    }
                    ...
                    public function onlineExamination_by_class()
                    {
                    $getStaffID               = $this->customlib->getStaffID();
                    ...
                    $data=array();
                    $this->session->set_userdata('top_menu', 'manage_application');
                    $this->session->set_userdata('sub_menu', 'manage_application/exams_by_session');
                    ...
                    $data['classList']         = $this->class_model->get();
                    $class_id                  = $this->input->post('class_id');
                    $data['subjectlist']       = $this->onlineexam_model->get_subjectlist_byclass($exam_group_id,$exam_id,$session_id,$class_id,$section_id,$getStaffID);

                    $data['class_by_teacher']  = $this->onlineexam_model->get_class_by_teacher($getStaffID); 
                    }
                    $this->load->view('layout/header', $data);
                    $this->load->view('admin/onlineexam/online_examination_byClass', $data);
                    $this->load->view('layout/footer', $data);
                    }  
                    ...
                    public function list_onlineexam ()
                    {                    
                    ...
                    $data['getstudent_workingdays']  =   $this->examgroup_model->adminview_attendence($studentid);
                    $data['getstudent_presentdays']  =   $this->examgroup_model->adminview_Studentattendence($studentid);
                    
                    $data['getstudent_latedays']     =   $this->examgroup_model->adminview_Studentattendence_late($studentid);
                    
                    $data['getstudent_halfdays']     =   $this->examgroup_model->view_Studentattendence_halfdays($studentid);
                    
                    // $data['get_getsubjectlist']  =   $this->examgroup_model->get_getsubjectlist($examgroup,$exambatch,$studentid);
                    
                    $data['subjects_row']            =   $this->examgroup_model->get_subjectdetails_row($examgroup,$exambatch,$studentid);
                    $data['online_exam_accept'] = $this->examgroup_model->online_exam_accept($examgroup,$exambatch,$studentid);
                    
                    
                    $data['online_exam_accept_result'] = $this->examgroup_model->online_exam_accept_result(); 
                    ...
                    $this->load->view('admin/onlineexam/online_exam_ajax',$data);                    
                    }
                    ...
                    public function viewsubjectpdf($studid,$group,$batchid)
                    {
                    ...
                    //$this->pdf->loadHtml(utf8_decode($html_content));
                    ...
                    public function viewsubjectpdf_print($studid,$group,$batchid)
                    {
                    ...
                    $array = array('status' => '1', 'error' => '', 'page' => $html_content);
                    echo json_encode($array);
                    ...
                    public function accept_exam($studid,$groupid,$exambatchid)
                    { 
                    ...
                    $studentval                 =   $this->onlineexam_model->getstudent($studid);
                    $data['examschedule']       = $this->batchsubject_model->getexamgroup_And_exam_Name($exambatchid,$groupid);
                    $examschedule               = $data['examschedule'] ;
                    $stu=array($studentval['session_id']);
                  
                    if ( $q->num_rows() > 0 ) 
                    {
                    $condition          = array('online_examination_examgroup'     => $groupid,'online_examination_exam'     => $exambatchid,'online_examination_student_id'     => $studid);
                    $data               = array('online_examination_subject_status'   => 2,'online_examination_updateddate'=>date('d-m-Y'));
                    
                    // $this->mailsmsconf->mailsms('exam_registration', $stu, $date, $examschedule); 
                    $this->db->where($condition);
                    $this->db->update('online_examination_accept', $data);
                    ...
                    'online_examination_student_id'  => $studid,
                    'online_examination_subject_status'  => 2,
                    'online_examination_date'=>date('d-m-Y'));
                    
                    $this->db->insert('online_examination_accept',$data);
                    ...
                    public function accept_ajaxexam()
                    {
                    ...
                    $data               = array('online_examination_subject_status'   => 2);
                    ...
                    $da=$this->db->insert('online_examination_accept',$data);
                    $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">' . $this->lang->line('success_message') . '</div>');
                    ...
                    public function reject_exam($studid,$groupid,$exambatchid)
                    {
                    ...
                    $data               = array('online_examination_subject_status'   => 3);
                    ...
                    'online_examination_exam'        => $exambatchid,        
                    'online_examination_student_id'  => $studid ,
                    'online_examination_subject_status'  => 3
                    );
                    ...
                    public function viewsubjectpdf_byadmin()
                    {
                    $data = array();
                    ...
                    $data['subjects']           =   $this->onlineexam_model->getadminsubjectdetails($students_array,$post_exam_id,$post_exam_group_id);
                    
                    $data['exam_subjects_row']  = $this->batchsubject_model->getExamSubjects_row($post_exam_id);
                    
                    $data['subjects_row']       = $this->batchsubject_model->getexamgroup_And_exam_Name($post_exam_id,$post_exam_group_id);
                    ...
                    $data['exam_subjects_SayExam']     = $this->examstudent_model->say_exam($post_exam_id,$post_exam_group_id);
                    
                    // $data['applicationsubject']        = $this->examstudent_model->getstudent_applicationsubject($post_exam_id,$post_exam_group_id,$class_id,$section_id,$session_id );
                    
                    
                    //$data['bycategory']              =$this->examstudent_model->getstudent_bycategory($student_id,$group,$batchid,$class_id,$section_id,$session_id);
                    
                    
                    $data['getstudent_workingdays']  =   $this->onlineexam_model->get_attendence($students_array);
                    $data['getstudent_presentdays']  =   $this->onlineexam_model->view_Studentattendence($students_array);
                    $data['getstudent_halfdays']     =   $this->onlineexam_model->view_Studentattendence_halfdays($students_array);                
                    $data['getstudent_latedays']     =   $this->onlineexam_model->view_Studentattendence_late($students_array);
                    ...
                    //$data['applicationfee']           =  $this->examstudent_model->getstudent_applicationfee($students_array);
                    
                    
                    
                    $student_admit_cards = $this->load->view('admin/onlineexam/_online_examinationadminviewpdf', $data, true);
