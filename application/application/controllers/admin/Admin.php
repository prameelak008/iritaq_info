...
class Admin extends Admin_Controller
{
    ...
    public function dashboard()
    {
        ...
        if ($data['sch_setting']->attendence_type == 0) {
          ...
          $current_date                        = date('Y-m-d');
        $data['leave_request']                 = $this->staff_model->staff_leave_request($current_date);

         $data['leave_request_holiday']                 = $this->staff_model->staff_leave_request_holiday($current_date);

        $data['leave_request_absent']          = $this->staff_model->staff_leave_request_absent($current_date);
       ...
       // $data['staff_absent_request']           = $this->staff_model->staff_absent_request($current_date);
      ...
      // $data['classes']                       = $this->staff_model->classes();
        $data['student_leave_request']         = $this->staff_model->student_leave_request($current_date);


         $data['student_leave_request_holiday'] = $this->staff_model->student_leave_request_holiday($current_date);




        $data['student_leave_request_absent']  = $this->staff_model->student_leave_request_absent($current_date);
        
        
        $data['total_online_examination_currentsession']        = $this->staff_model->total_online_examination_currentsession();
        $data['accepted_online_examination_currentsession']     = $this->staff_model->accepted_online_examination_currentsession();
        $data['rejected_online_examination_currentsession']     = $this->staff_model->rejected_online_examination_currentsession(); 
     ...
     public function dtstudentlist($search_text)
    {
       ...
       $currency_symbol = $this->customlib->getSchoolCurrencyFormat();
        ...
        $data['title'] = 'Our terms And Condition';


        $this->session->set_userdata('top_menu', 'Termsandcondition');
        $this->session->set_userdata('sub_menu', 'Termsandcondition/Termsandcondition');        

        $this->load->view('layout/header');
        $this->load->view('admin/termsandcondition/payment_terms');
        ...
        public function entrancedashboard()
        {
