...
if (!defined('BASEPATH')) {
        ...
        $staff_leaves_previous = $this->leaverequest_model->staff_leave_request_previous($id);
        
        
        $alloted_leavetype     = $this->staff_model->allotedLeaveType($id);
        
        $data['sch_setting']   = $this->sch_setting_detail;
        $sch_sett              = $data['sch_setting'] ;
        ...
        $sess_val             =  explode('-',$sch_sett->session);
        $sess_yr              =  $sess_val[0];
        ...
        $leaveDetail[$i]['alloted_leavehours'] = $value["alloted_leavehours"];
        ...
        $data["staff_leaves_previous"] = $staff_leaves_previous;
        ...
        $data['startMonth_val']      = $this->setting_model->getStartMonth();
        $session                     = $this->setting_model->getCurrentSessionName();
        $session_start               = explode("-", $session);
        $start_year                  = $session_start[0];
        $data['st_year_val']         = $start_year;
        $st_year_val                 = $data['st_year_val'];
        $date                        = $start_year . "-" . $startMonth;
        $newdate                     = date("Y-m-d", strtotime($date . "+1 year")-1);
        $nw                          = explode('-',$newdate);
        $data['currentyear_val']     = $nw[0];
        $data['current_mnth_val']    = $nw[1];
        $currentyear_val             = $data['currentyear_val'];
        $current_mnth_val            = $data['current_mnth_val'];
        
        $data['staff_payroll_val']            = $this->staff_model->getStaffPayroll_bymonth_val($id,$startMonth,$st_year_val);
        $data['staff_payroll_previous']       = $this->staff_model->getStaffPayroll_previous($id,$startMonth,$st_year_val);
        ...
        $data_insert['shortname'] = $shortname;
        }
        ...
        $leave_typehrs  = $this->input->post('leave_typehrs');
        $leave_array = array();
        $leavehrs_array = array();
        ...
        'alloted_leavehours' => $this->input->post('alloted_leavehrs_' . $leave_value),  
        );
        }
        }
        // if (empty($leavehrs_array)) {
        //     foreach ($leave_typehrs as $leave_keyhrs => $leave_valuehrs) {
        //         $leavehrs_array[] = array(
        //             'staff_id'      => 0,
        //             'leave_type_id' => $leave_valuehrs,
        //             'alloted_leavehours' => $this->input->post('alloted_leavehrs_' . $leave_valuehrs),   
        //         );
        //     }
        // }
        ...
        $data3 = array('staff_id' => $id,
        'leave_type_id'           => $leave_typehrs[$i],
        'id'                      => $altidhrs[$i],
        'alloted_leavehours'      => $alloted_leavehours[$i],
        );
        ...
        */
        ...
        $leave_typehrs    = $this->input->post("leave_type_idhrs");
        $alloted_leavehours = $this->input->post("alloted_leavehours");
        $altidhrs         = $this->input->post("altidhrs");
        if (!empty($leave_type)) {
        $i = 0;
        foreach ($leave_type as $key => $value) {
        ...
        $this->staff_model->add_staff_leave_details($data2,$data3);
        // $this->staff_model->add_staff_leave_detailshrs($data3);
        $i++;
        }
        }
        ...
        $userdata              = $this->customlib->getUserData();
        $leave_request         = $this->leaverequest_model->user_leave_request($userdata["id"],$current_session);
        
        $leave_request_previous= $this->leaverequest_model->user_leave_request_previous($userdata["id"],$current_session);
