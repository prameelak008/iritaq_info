...
class Student extends Admin_Controller
{
    ...
    public function Newexportfeeformat()
    {
        $this->load->helper('download');
        $filepath = "./backend/import/New_export_fees_Format.csv";
        $data     = file_get_contents($filepath);
        $name     = 'New_export_fees_Format.csv';
	...
	{
	$data['logindetails']    = $this->student_model->Newlistlogindetails();
    ...
    $this->load->view('student/studentLoginList', $data);
    ...
    public function handle_studentsign_upload()
    {
        ...
        if (isset($_FILES["studentsign"]) && !empty($_FILES['studentsign']['name'])) {
            ...
            $file_type = $_FILES["studentsign"]['type'];
            $file_size = $_FILES["studentsign"]["size"];
            $file_name = $_FILES["studentsign"]["name"];

            $allowed_extension = array_map('trim', array_map('strtolower', explode(',', $result->image_extension)));
            $allowed_mime_type = array_map('trim', array_map('strtolower', explode(',', $result->image_mime)));
            $ext               = strtolower(pathinfo($file_name, PATHINFO_EXTENSION));

            if ($files = @getimagesize($_FILES['studentsign']['tmp_name'])) {

                if (!in_array($files['mime'], $allowed_mime_type)) {
                    $this->form_validation->set_message('handle_studentsign_upload', 'File Type Not Allowed');
                    return false;
                }

                if (!in_array($ext, $allowed_extension) || !in_array($file_type, $allowed_mime_type)) {
                    $this->form_validation->set_message('handle_studentsign_upload', 'Extension Not Allowed');
                    return false;
                }
                if ($file_size > $result->image_size) {
                    $this->form_validation->set_message('handle_studentsign_upload', $this->lang->line('file_size_shoud_be_less_than') . number_format($image_validate['upload_size'] / 1048576, 2) . " MB");
                    return false;
                }
            } else {
                $this->form_validation->set_message('handle_studentsign_upload', "File Type / Extension Error Uploading  Image");
                return false;
            }
...
/*
	   ...
	   $this->student_model->addnewsiblings($st,$sibl);
          ...
          redirect($_SERVER['HTTP_REFERER']);
					...
					public function newstudentdelete()
					{
					$id             = $this->input->post('id');
                    $this->db->where('childs', $id);      
                    $q = $this->db->get('users');
					$reco 		    = $q->row_array();			
					$parentid	    = $reco['id'];
					...
					$this->db->where('id', $id); 
					$array=$this->db->update('students', $data);
					...
					public function newcheckifstudentexists()
					{
					$student_id             = $this->input->post('student_id');	
					$studentuser_id         = $this->input->post('studentuser_id');
					$this->db->where('id', $studentuser_id);      
					$q 		    = $this->db->get('students');
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            $resultlist          = $this->student_model->searchByClassSectionbulk($class, $section);
    ...
    public function getByClassAndSectionExcludeMe()
    {
	 ...
	 public function getByNewClassAndSectionExcludeMe()
    {
        $class      = $this->input->post('class_id');
        $section    = $this->input->post('section_id');
        $student_id = $this->input->post('current_student_id');
        $resultlist = $this->student_model->searchByClassSectionWithoutCurrent($class, $section, $student_id);
...
//datatable function to check search parameter validation
    ...
    public function getStudentByClassSection()
    {
		...
		$usersData = $this->student_model->exportExcel($class_id,$section_id,$namelist);
		...
		$header = array("Institutional Id","firstname","lastname","class","section","Username","Password","Mobile");
		...
		$data['logindetails'] =   $this->student_model->NewlistlogindetailsId($class_id,$section_id,$namelist);			
		...
		$this->load->view('student/studentajxbyclass',$data);
        ...
        $class_id   = $this->input->post('class_id');
        $section_id = $this->input->post('section_id');
        $data       = $this->section_model->getNameBySection($class_id,$section_id);
...
//////////////////////////////////////////////////////////////////////////////////////////////
        ...
        $this->load->view('student/examstudentShow', $data);
        $this->load->view('layout/footer', $data);
        }

    public function examviewByName()
    {
        $sid = $this->input->post('admission_id');
        $adlist = $this->class_model->examgetById($sid);
        $data['admissionlist'] = $adlist;
        $adlistnames = $this->class_model->examgetAllnames();
        $data['adlistnames'] = $adlistnames;
        $this->load->view('layout/headerentrance', $data);
        $this->load->view('student/examstudentSearchByname', $data);
        $this->load->view('layout/footer', $data);
    }
...
public function examsearch()
    {
        ...
        $this->load->view('layout/headerentrance', $data);
        $this->load->view('student/examstudentSearch', $data);
