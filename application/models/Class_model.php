...
class Class_model extends MY_Model {
  ...
  ////////////////////////////////////////// //Entrance Exam/////////////////////// ///////////////////////////
                ...
                public function examgetById($id) {
                $query = $this->db->select('*')->join("entrance_examregister", "entrance_examregister.entrance_reg_id = admission_form_tbl.admission_application_registerid")->join("entranceexam_course", "entranceexam_course.entranceexam_course_id=admission_form_tbl.admission_application_selectedcourse")->join("state_list", "state_list.state_id=admission_form_tbl.admission_state")->join("district_list", "district_list.district_id =admission_form_tbl.admission_district")->where("admission_form_tbl.admission_id", $id)->get("admission_form_tbl");
                return $query->result_array();
                
                }
                public function examgetAllnames() 
                {
                $query = $this->db->select('*')->join("entrance_examregister", "entrance_examregister.entrance_reg_id = admission_form_tbl.admission_id")->get("admission_form_tbl");
                return $query->result_array();
                
                }
                
                
                public function examget($id = null, $classteacher = null) {
                ...
                $this->db->select()->from('admission_form_tbl');
                if ($id != null) {
                $this->db->where('admission_id', $id);
                } else {
                $this->db->order_by('admission_id');
                }
                $query = $this->db->get();
                if ($id != null) {
                $classlist = $query->row_array();
