...
class Librarymanagement_model extends MY_Model {
    ...
    public function add_library($data) 
    {
        ...
        if (isset($data['id'])) {
            $this->db->where('id', $data['id']);
            $this->db->update('sem_libarary_members', $data);
            $message = UPDATE_RECORD_CONSTANT . " On  sem_libarary_members members id " . $data['id'];
            $action = "Update";
            $record_id = $insert_id = $data['id'];
            $this->log($message, $record_id, $action);
        } else {
            $this->db->insert('sem_libarary_members', $data);
            $insert_id = $this->db->insert_id();
            $message = INSERT_RECORD_CONSTANT . " On sem_libarary_members members id " . $insert_id;
            $action = "Insert";
            $record_id = $insert_id;
            $this->log($message, $record_id, $action);
        }
    ...
    function check_data_exists($data) {
     ...
     function check_data_sem_exists($data) {
        $this->db->where('library_card_no', $data['library_card_no']);
        $query = $this->db->get('sem_libarary_members');
        if ($query->num_rows() > 0) {
            return $query->row();
        } else {
            return FALSE;
