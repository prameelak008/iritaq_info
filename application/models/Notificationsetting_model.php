...
class Notificationsetting_model extends MY_Model
{
    ...
    public function get_online($id = null)
    {
    $this->db->select()->from('online_notification_setting');
    
    if ($id != null) 
    {
    $this->db->where('online_notification_setting.id', $id);
    } 
    else
    {
    $this->db->order_by('online_notification_setting.id');
    }
    ...
    // public function updatebatch_online($update_array)
    // {
    ...
    // if (isset($update_array) && !empty($update_array)) 
    // {
    // $this->db->update_batch('online_notification_setting', $update_array, 'id');
    // }
    
    // foreach($update_array as $ua)
    // {
    // $message      =  UPDATE_RECORD_CONSTANT." On online_notification_setting setting id ".$ua['id'];
    // $action       = "Update";
    // $record_id    = $ua['id'];
    // $this->log($message, $record_id, $action);
    // }
    ...
    // }
            ...
            $this->db->trans_start(); // Starting Transaction
            $this->db->trans_strict(false); // Optional strict mode, can be removed
            
            //======================= Code Start ===========================
            if (!empty($update_array) && is_array($update_array)) 
            {
            ...
            if ($this->db->trans_status() === false) 
            {
            $this->db->trans_rollback(); // Rollback if something went wrong
            return false;
            } 
            else
            {
            $this->db->trans_commit(); // Commit if everything is fine
            return true;
            }
            }
            ...
            //=======================Code Start===========================
            $this->db->where('id', $data['id']);
            $this->db->update('online_notification_setting', $data);
