            ...
            public function add_subs($delete_array, $insert_array, $update_array,$subst_date) 
            {
            $this->db->trans_start(); # Starting Transaction
            $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
            if (!empty($delete_array)) 
            ...
            $this->db->where_in('timetable_id', $delete_array);
            $this->db->delete('subject_timetable_substitute');
            }
            if (isset($update_array) && !empty($update_array)) 
            {
            $this->db->where(array('substitute_date'=>$subst_date));
            $this->db->update_batch('subject_timetable_substitute', $update_array, 'timetable_id');
            }
            if (isset($insert_array) && !empty($insert_array)) 
            {

            $this->db->insert_batch('subject_timetable_substitute', $insert_array);
            $count = count($insert_array);
            $id = $this->db->insert_id();
            }
            ...
            {
            $this->db->select('subjects.name as subname,subject_timetable.*');
            $this->db->from('subject_timetable');
            $this->db->join('subject_group_subjects', 'subject_timetable.subject_group_subject_id = subject_group_subjects.id');
            // $this->db->join('subject_group_subjects', 'subject_timetable.subject_group_subject_id = subject_group_subjects.subject_id');
            $this->db->join('staff', 'staff.id       = subject_timetable.staff_id');
            $this->db->join('subjects', 'subjects.id = subject_group_subjects.subject_id');
            //$this->db->join('subjectpaper', 'subjectpaper.subjectpaper_subjectid = subjects.id');
            ...
            public function getByDayClassSection($day, $class_id, $section_id,$subject_group_id) 
            {
            $this->db->select('periodic_table.periodic_table_description as period,subjects.name as subjectname,subjectpaper.subjectpaper_papercode as papercode,subjectpaper.subjectpaper_papername as papername,staff.name as staffname,staff.surname as surname,subject_timetable.id as sid,subject_timetable.id as sid,subject_timetable.*');
            $this->db->from('subject_timetable');
            $this->db->join('subjectpaper', 'subjectpaper.subjectpaper_id = subject_timetable.subject_paper_id','left');
            $this->db->join('subjects', 'subjects.id = subjectpaper.subjectpaper_subjectid','left');
            $this->db->join('staff', 'staff.id = subject_timetable.staff_id');
            $this->db->join('periodic_table', 'periodic_table.periodic_table_id  = subject_timetable.period_id');
            $this->db->where(array('subject_timetable.class_id'=> $class_id));
            $this->db->where('subject_timetable.section_id', $section_id);
            $this->db->where('subject_timetable.day', $day);
            // $this->db->where('subject_timetable.subject_group_id', $subject_group_id);
            ...
            public function getBySubjectGroupDayClassSection_substitute($subject_group_id, $day, $class_id, $section_id) 
            {
            $this->db->select('subject_timetable.*,staff');
            $this->db->from('subject_timetable');
            $this->db->join('subject_group_subjects', 'subject_timetable.subject_group_subject_id = subject_group_subjects.id');
            $this->db->join('staff', 'staff.id = subject_timetable.staff_id');
            $this->db->where('subject_timetable.class_id', $class_id);
            ...
            $sql = "SELECT `subject_group_subjects`.`subject_id`,subjects.id as `subj_id`,subjects.name as `subject_name`,subjects.code,subjects.type,staff.name,staff.surname,staff.shortname,staff.employee_id,periodic_table.periodic_table_name,`subject_timetable`.* FROM `subject_timetable`  JOIN `subject_group_subjects` ON `subject_timetable`.`subject_group_subject_id` = `subject_group_subjects`.`id` inner JOIN subjects on subject_group_subjects.subject_id = subjects.id      INNER JOIN staff on staff.id=subject_timetable.staff_id   JOIN `periodic_table` ON `subject_timetable`.`period_id` = `periodic_table`.`periodic_table_id`   WHERE `subject_timetable`.`class_id` = " . $class_id . " AND `subject_timetable`.`section_id` = " . $section_id . " AND `subject_timetable`.`day` = " . $this->db->escape($day) . " AND `subject_timetable`.`session_id` = " . $this->current_session . "" . $subject_condition;

            $query = $this->db->query($sql);
            return $query->result();
            }

            public function getclasssection($class_id, $section_id)
            {
            $this->db->select('*');
            $this->db->from('subject_timetable');
            $this->db->join('classes','classes.id=class_id');
            $this->db->join('sections','sections.id=section_id');
            $this->db->where('subject_timetable.class_id',$class_id);
            $this->db->where('subject_timetable.section_id',$section_id);
            $this->db->where('subject_timetable.session_id',$this->current_session);
            $this->db->group_by('subject_timetable.class_id');
            $query=$this->db->get();
            return $query->row();
            }
            ...
            {
            $this->db->where(array('periodic_table_id'=> $id));      
            $q   = $this->db->get('periodic_table');
            return $q->row_array();
            }
            ...
            {
            $this->db->where(array('periodic_table_session'=>  $this->current_session,'periodic_table_status'=>1));      
            $q   = $this->db->get('periodic_table');
            return $q->result();
            }
            ...
            public function addsubject_paper($subjectpaper_id)
            {
            $this->db->select('*,subjects.name as subjectname');
            $this->db->from('subjects');
            $this->db->join('subjectpaper', 'subjectpaper.subjectpaper_subjectid = subjects.id');
            $this->db->where(array('subjectpaper.subjectpaper_session'=>$this->current_session,'subjectpaper.subjectpaper_id'=> $subjectpaper_id));
            $query = $this->db->get();
            return $query->result();
            }


            public function get_subjectpaper($subjectpaper_id)
            {
            $this->db->select('*');
            $this->db->from('subjects');
            $this->db->join('subjectpaper', 'subjectpaper.subjectpaper_subjectid = subjects.id');
            $this->db->where(array('subjectpaper.subjectpaper_id'=> $subjectpaper_id));
            $query = $this->db->get();
            return $query->row();
            }
            ...
            public function get_subjectsetpapers()
            {
            $this->db->select('*');
            $this->db->from('subjectpaper');
            $this->db->where(array('subjectpaper.subjectpaper_session'=>$this->current_session));
            $query = $this->db->get();
            return $query->result_array();
            }
            ...
            {
            $this->db->select('subject_timetable_substitute.*,staff.shortname,subject_group_subjects.subject_id,subjects.name,subjects.code,subjects.type');
            $this->db->from('subject_timetable_substitute');
            $this->db->join('subject_group_subjects', 'subject_timetable_substitute.subject_group_subject_idsubs = subject_group_subjects.id');
            $this->db->join('staff', 'staff.id = subject_timetable_substitute.staff_idsubs');

            $this->db->join('subjects', 'subjects.id = subject_group_subjects.subject_id');
            //$this->db->where(array('subject_timetable_substitute.timetable_id'=> $prev_rec_value->id,'subject_timetable_substitute.substitute_date'=> $subst_date));
            $query = $this->db->get();
            return $query->result();
            }
            ...
            public function getclasssection_substituteReport($class_id, $section_id,$subject_group_id, $subst_date)
            {
            if($class_id!="" && $section_id!="" && $subject_group_id!="" && $subst_date!="")
            {
            $this->db->select('subject_timetable_substitute.*,staff.name as staffname,periodic_table.periodic_table_name as periodname,subjects.name as subjectname,subjects.code as subjectcode,classes.class as Classname,sections.section as Sectionname');
            $this->db->from('subject_timetable_substitute');
            $this->db->join('subject_group_subjects', 'subject_timetable_substitute.subject_group_subject_idsubs = subject_group_subjects.id');
            $this->db->join('staff', 'staff.id = subject_timetable_substitute.staff_idsubs');
            $this->db->join('subjects', 'subjects.id = subject_group_subjects.subject_id');
            $this->db->join('periodic_table', 'periodic_table.periodic_table_id = subject_timetable_substitute.period_id');
            $this->db->join('classes','classes.id=subject_timetable_substitute.class_id');
            $this->db->join('sections','sections.id=subject_timetable_substitute.section_id');
            $this->db->where('subject_timetable_substitute.class_id',$class_id);
            $this->db->where('subject_timetable_substitute.section_id',$section_id);
            $this->db->where('subject_timetable_substitute.subject_group_id', $subject_group_id);
            $this->db->where('subject_timetable_substitute.session_id',$this->current_session);
            $this->db->where('subject_timetable_substitute.substitute_date',$subst_date);
            $query = $this->db->get();
            return $query->result_array();
            }
            else
