        ...
        {
        $this->db->select('student_session.session_id,student_session.transport_fees,students.app_key,students.studentsign,students.parent_app_key,students.vehroute_id,vehicle_routes.route_id,vehicle_routes.vehicle_id,transport_route.route_title,vehicles.vehicle_no,hostel_rooms.room_no,vehicles.driver_name,vehicles.driver_contact,hostel.id as `hostel_id`,hostel.hostel_name,room_types.id as `room_type_id`,room_types.room_type ,students.hostel_room_id,student_session.id as `student_session_id`,student_session.fees_discount,classes.id AS `class_id`,classes.class,sections.id AS `section_id`,sections.section,students.id,students.admission_no , students.roll_no,students.admission_date,students.firstname,students.middlename,  students.lastname,students.image,    students.mobileno, students.email ,students.state ,   students.city , students.pincode , students.note, students.religion, students.cast, school_houses.house_name,   students.dob ,students.current_address, students.previous_school,
        students.guardian_is,students.parent_id,
        students.permanent_address,students.category_id,students.wasupno,students.passportno,students.adhar_no,students.samagra_id,students.bank_account_no,students.bank_name, students.ifsc_code , students.guardian_name , students.father_pic ,students.height ,students.weight,students.measurement_date, students.mother_pic , students.guardian_pic , students.guardian_relation,students.guardian_phone,students.guardian_address,students.is_active ,students.created_at ,students.updated_at,students.father_name,students.father_phone,students.blood_group,students.school_house_id,students.father_occupation,students.mother_name,students.mother_phone,students.mother_occupation,students.guardian_occupation,students.gender,students.guardian_is,students.rte,students.guardian_email, users.username,users.password,students.dis_reason,students.dis_note,students.disable_at,students.last_studied_madrasa,students.entrance_reg_id,students.last_studied_madrasa_class,students.previous_reg_no,students.last_studied_school,students.last_studied_class,students.previous_medium,students.last_studied_institute,students.years_completed,students.previous_place,students.name_of_prominent_teacher,students.major_books_studied,students.general_education')->from('students');
        $this->db->join('student_session', 'student_session.student_id = students.id');
        $this->db->join('classes', 'student_session.class_id = classes.id');
        $this->db->join('sections', 'sections.id = student_session.section_id');
        $this->db->join('hostel_rooms', 'hostel_rooms.id = students.hostel_room_id', 'left');
        ...
        public function searchByClassSection($class_id = null, $section_id = null,$session)
        {
        $i = 1;
        ...
        $this->db->join('sections', 'sections.id = student_session.section_id');
        $this->db->join('categories', 'students.category_id = categories.id', 'left');
        $this->db->where(array('student_session.session_id'=> $session));
        ...
        public function searchByClassSectionbulk($class_id = null, $section_id = null)
        {
        $i = 1;
        ...
        public function searchByClassSectionRepo($class_id = null, $section_id = null)
        {
        $i = 1;
        ...
        public function searchByClassSection_admitteemplate($exam_group_id, $exam_id,$class_id = null, $section_id = null,$session)
        {
        ...
        $this->db->select('exam_group_class_batch_exam_students.exam_qrcode as exam_qrcode,classes.id AS `class_id`,student_session.id as student_session_id,students.id,classes.class,sections.id AS `section_id`,sections.section,students.id,students.admission_no , students.roll_no,students.admission_date,students.firstname,students.middlename,  students.lastname,students.image,    students.mobileno, students.email ,students.state ,   students.city , students.pincode ,     students.religion,     students.dob ,students.current_address,    students.permanent_address,IFNULL(students.category_id, 0) as `category_id`,IFNULL(categories.category, "") as `category`,students.adhar_no,students.samagra_id,students.bank_account_no,students.bank_name, students.ifsc_code , students.guardian_name , students.guardian_relation,students.guardian_phone,students.guardian_address,students.is_active ,students.created_at ,students.updated_at,students.father_name,students.app_key,students.parent_app_key,students.rte,students.gender,' . $field_variable)->from('students');
        $this->db->join('student_session', 'student_session.student_id = students.id');
        ...
        public function addnewsiblings($st,$sbl)
        {
        
        
        $this->db->where('childs', $st);      
        $q = $this->db->get('users');
        
        $this->db->where('user_id', $sbl);
        $this->db->where('childs', $sbl);
        ...
        $this->db->where('id', $st);      
        $qt = $this->db->get('students');		
        
        $re = $qt->row_array();
        $pid=$re['parent_id'];		
        
        $this->db->where('id', $sbl);      
        $qoo = $this->db->get('students');
        
        $rec_idd  = $qoo->row_array();	
        $studp    = $rec_idd['parent_id'];
        ...
        public function getMyNewChildrens($parent_id)
        {
        $this->db->select('*,student_session.id as sid');
        ...
        {
        /*$sql = "SELECT users.* FROM users WHERE id in (select students.parent_id from users INNER JOIN students on students.id =users.user_id WHERE 
        users.role ='student') UNION  select users.* from users INNER JOIN students on students.id =users.user_id 
        ";
        
        $sql = "SELECT students.firstname,users.username,users.password FROM users
        INNER JOIN students
        ON students.id =users.user_id WHERE 
        users.role ='student'";*/
        ...
        public function exportExcel($id,$section_id,$namelist)
        {			 
        if($id=='')
        {	
        
        $this->db->select('students.admission_no,students.firstname,students.firstname,students.lastname,classes.class,sections.section,users.username,users.password,students.mobileno');
        $this->db->from('students');
        $this->db->join('users', 'students.id = users.user_id', 'left');
        $this->db->join('student_session', 'student_session.student_id = students.id');
        $this->db->join('classes', 'student_session.class_id = classes.id');
        ...
        else if($id!="" && $section_id=="" && $namelist=="")
        {
        $this->db->select('students.admission_no,students.firstname,students.firstname,students.lastname,classes.class,sections.section,users.username,users.password,students.mobileno');
        $this->db->from('students');
        $this->db->join('users', 'students.id = users.user_id', 'left');
        ...
        public function getStudentsByArray_admittemplate($array)
        {
        $i             = 1;
        $custom_fields = $this->customfield_model->get_custom_fields('students');
        ...
        ->searchable('students.admission_no,students.firstname,classes.class,students.father_name,students.dob,students.admission_date,students.gender,categories.category,students.mobileno,students.adhar_no'.$field_variable)
        ->join('student_session', 'student_session.student_id = students.id')
        ->join('classes', 'student_session.class_id = classes.id')
        ->join('sections', 'sections.id = student_session.section_id')
        ->join('categories', 'students.category_id = categories.id', 'left')
        ->where('student_session.session_id', $this->current_session)
        ->where('students.is_active', "yes")
        ->orderable('students.admission_no,students.firstname,classes.class,students.father_name,students.dob,students.gender,categories.category,students.mobileno,students.adhar_no'.$field_name)
        ->from('students');
        ...
        public function getAllExamList($id)
        {
        $this->db->select('*,exam_groups.name as groupname,exam_group_class_batch_exams.id as exm_id,subjects.name as subname,exam_group_class_batch_exam_subjects.max_marks as maxmark,exam_group_exam_results.get_marks as marks,exam_group_class_batch_exam_students.teacher_remark as teacherremarks,exam_group_exam_results.attendence as attendence');
        ...
        $this->db->where('exam_group_class_batch_exam_students.student_id', $id);
        ...
        public function gethalfyearExamList($id)
        {
        
        $this->db->select('*,subjects.name as subname,exam_group_class_batch_exam_subjects.max_marks as maxmark,exam_group_exam_results.get_marks as marks,exam_group_class_batch_exam_students.teacher_remark as teacherremarks,exam_group_exam_results.attendence as attendence');
        $this->db->from('exam_groups');
        $this->db->join('exam_group_class_batch_exams', 'exam_group_class_batch_exams.exam_group_id = exam_groups.id');
        $this->db->join('exam_group_class_batch_exam_students', 'exam_group_class_batch_exam_students.exam_group_class_batch_exam_id = exam_group_class_batch_exams.id');
        ...
        $this->db->where('exam_group_class_batch_exam_students.student_id', $id);
        $this->db->where('exam_group_class_batch_exams.exam','HALF YEARLY EXAMINATION');
        ...
        public function getannualExamList($id)
        {
        ...
        $this->db->where('exam_group_class_batch_exam_students.student_id', $id);
        $this->db->where('exam_group_class_batch_exams.exam','ANNUAL EXAMINATION');
        ...
        public function Newgetemailconfig()
        {
        $this->db->select('*')->from('email_config');        
        $this->db->where('email_config.id', 1);
        $query = $this->db->get();
        return $query->row_array();
        }  
        
        
        public function Newgetstudentlist($idd)
        {
        $this->db->select('*');
        $this->db->from('students');
        $this->db->join('student_session', 'student_session.student_id = students.id');
        $this->db->where(array('student_session.id'=> $idd));
        $query = $this->db->get();
        return $query->row_array();
        } 
        ...
        {
        $this->db->select('*');
        $this->db->from('progress_card_layout_tbl');
        $this->db->where(array('progress_card_id'=> 1));
        $this->db->order_by('progress_card_autoid', 'desc');
        $this->db->limit(1);
        $query = $this->db->get();
        return $query->row_array();
        }
        ...
        {
        $this->db->select('*,sessions.id as sesidd,sessions.session as sesname,
        classes.id as classesidd,classes.class as classesname,
        sections.id as sectionsidd,sections.section as sectionsname,students.id as studid,students.firstname as studname');
        $this->db->from ('students');
        $this->db->join('student_session', 'student_session.student_id = students.id');
        ...
        public function getStudentListBYStudentID($idd)
        {
        $this->db->select('*');
        $this->db->from('students');
        $this->db->join('student_session', 'student_session.student_id = students.id');
        $this->db->join('sessions', 'sessions.id = student_session.session_id');
        $this->db->where(array('student_session.id'=> $idd,'student_session.session_id'=> $this->current_session));
        $query = $this->db->get();
        return $query->result();
        }
        ...
        public function get_max_date_to_from_exam($examgroup,$exam_id,$session)
        {
        $this->db->select('online_examination_lasdate_of_exam  as max_to_date_formatted');
        $this->db->from('online_examination_instruction');
        $this->db->where(array('online_examination_instruction.online_examination_examgroup' => $examgroup,'online_examination_instruction.online_examination_examid' => $exam_id, 'online_examination_instruction.online_examination_current_session' => $session));
        $query = $this->db->get();
        return $query->row_array();
        }
        
          ///////........................Collect Fees Balance Display  
        ...
        {
        $this->db->select('student_fees_master.id, student_fees_deposite.amount_detail');
        ...
        $this->db->where('student_fees_master.student_session_id', $id);
        $query = $this->db->get();
        ...
        $details = json_decode($row->amount_detail, true);
        foreach ($details as $item) {
        $dis_amount += (float) $item['discount_amt'];
        }
        }
        return $dis_amount; 
        }      
        ...
        public function get_previousbal($id)
        {
        $this->db->select('SUM(student_fees_master.amount) AS t_amount');
        $this->db->from('student_fees_master');
        $this->db->join('fee_groups_feetype', 'fee_groups_feetype.fee_session_group_id = student_fees_master.fee_session_group_id');
        
        $this->db->where('student_fees_master.student_session_id', $id);
        $this->db->where('fee_groups_feetype.fee_groups_id', 13);  
        $query = $this->db->get();
        return $query->row(); // returns ->t_amt
        }  
