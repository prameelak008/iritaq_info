...
class User_model extends MY_Model {
    ...
    public function get_studentdefaultClass($student_id){
        ...
        ////////////////////////
            ...
            public function checkLogin_auth($data)
            {
            $this->db->select('id, username, password,role,is_active,lang_id');
            $this->db->from('sem_users');
            $this->db->where('username', $data['username']);
            $this->db->where('password', ($data['password']));
            $this->db->limit(1);
            $query = $this->db->get();
            ...
            return false;
                ...
                public function checkLoginParent_auth($data) 
                {
                $sql = "SELECT sem_users.*, students.admission_no, students.guardian_name, students.roll_no, students.admission_date,
                students.firstname, students.middlename, students.lastname, students.image, students.father_pic,
                students.mother_pic, students.guardian_pic, students.guardian_relation, students.mobileno, students.email,
                students.state, students.city, students.pincode, students.religion, students.dob, students.current_address,
                students.permanent_address, students.gender
                FROM `sem_users`
                INNER JOIN `semester_students` AS students ON students.parent_id = sem_users.id
                LEFT JOIN (SELECT * FROM languages) AS languages ON languages.id = sem_users.lang_id
                WHERE sem_users.`username` = " . $this->db->escape($data['username']) . "
                AND sem_users.`password` = " . $this->db->escape($data['password']) . "
                LIMIT 0,1";
                ...
                public function read_user_information_auth($users_id) {
                $this->db->select('users.*,languages.language,students.firstname, students.middlename,students.image,students.lastname,students.guardian_name,students.gender');
                $this->db->from('users');
                $this->db->join('students', 'students.id = users.user_id');
                $this->db->join('languages', 'languages.id = users.lang_id', 'left');
