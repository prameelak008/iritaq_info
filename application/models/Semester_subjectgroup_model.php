            ...
            class Semester_subjectgroup_model extends MY_Model {
            ...
            */
            ...
            $this->db->update('semester_subject_groups', $data);
            ...
            $this->db->insert_batch('semester_subject_group_subjects', $subject_group_subject_Array);
            }
            ...
            $this->db->delete('semester_subject_group_subjects');
            }
            ...
            public function add($data, $subject_group) {
            ...
            $message = UPDATE_RECORD_CONSTANT . " On semester_subject_groups id " . $data['id'];
            $action = "Update";
            $record_id = $data['id'];
            ...
            $this->db->insert('semester_subject_groups', $data);
            $subject_group_id = $this->db->insert_id();

            $message = INSERT_RECORD_CONSTANT . " On semester_subject_groups id " . $subject_group_id;
            $action = "Insert";
            ...
            $this->db->select('semester_subject_groups.*')->from('semester_subject_groups');

            if ($id != null) {
            $this->db->where('semester_subject_groups.id', $id);
            } else {
            $this->db->order_by('semester_subject_groups.id', 'DESC');
            }
            $query = $this->db->get();
            ...
            $sql = "SELECT semester_subject_group_subjects.*,subjects.name,subjects.code,subjects.type FROM `semester_subject_group_subjects` INNER JOIN subjects on subjects.id=semester_subject_group_subjects.subject_id WHERE subject_group_id =" . $this->db->escape($subject_group_id) . " " . $subject_groupid_condition;
            ...
            //=======================Code Start===========================
            $this->db->where('id', $id);
            $this->db->delete('semester_subject_groups');
            $message = DELETE_RECORD_CONSTANT . " On semester_subject_groups id " . $id;
            $action = "Delete";
            $record_id = $id;
            $this->log($message, $record_id, $action);
            ...
            public function add_semesterpaper($data)
            {                   
            $this->db->trans_start(); # Starting Transaction
            $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
            //=======================Code Start===========================
            if (isset($data['sem_paper_id']))
            {
            $this->db->where('sem_paper_id', $data['sem_paper_id']);
            $this->db->update('semestersubjectpaper', $data);
            $message     = UPDATE_RECORD_CONSTANT . " On  	semestersubjectpaper subjectpaper_id " . $data['subjectpaper_id'];
            $action      = "Update";
            $record_id   = $data['sem_paper_id'];
            $this->log($message, $record_id, $action);
            //======================Code End==============================
            ...
            $this->db->insert('semestersubjectpaper', $data);
            $insert_id = $this->db->insert_id();
            $message = INSERT_RECORD_CONSTANT . " On semestersubjectpaper subjectpaper_id " . $insert_id;
            $action = "Insert";
            $record_id = $insert_id;
            $this->log($message, $record_id, $action);
            //======================Code End==============================
            ...
            public function get_subjectpaper($subjectpaper_id = null)
            {
            $this->db->select()
            ->from('semestersubjectpaper')
            ->join('subjects','subjects.id = semestersubjectpaper.sem_paper_subjectid');  
            if ($subjectpaper_id != null) {
            $this->db->where('semestersubjectpaper.sem_paper_id', $subjectpaper_id );
            } 
            else 
            {
            $this->db->order_by('semestersubjectpaper.sem_paper_id');
            }
            $query = $this->db->get();
            if ($subjectpaper_id != null) {
            return $query->row_array();
            ...
            $this->db->delete('semestersubjectpaper');
            $message = DELETE_RECORD_CONSTANT . " On semestersubjectpaper id " . $id;
            $action = "Delete";
            $record_id = $id;
            $this->log($message, $record_id, $action);
            $this->db->trans_complete();
            ...
            public function paper_code_exists($code, $exclude_id = null)
            {
            $this->db->from('semestersubjectpaper');
            $this->db->where('sem_paper_code', $code);
            if (!is_null($exclude_id)) {
            $this->db->where('sem_paper_id !=', $exclude_id);
            }
            return $this->db->count_all_results() > 0;
            }
            ...
            public function paper_name_exists($name, $exclude_id = null)
            {
            $this->db->from('semestersubjectpaper');
            $this->db->where('sem_paper_paper', $name);
            if (!is_null($exclude_id)) {
            $this->db->where('sem_paper_id !=', $exclude_id);
            }
            return $this->db->count_all_results() > 0;
