...
class Staffattendancemodel extends MY_Model {
    ...
    public function searchAttendanceReportbyhour($user_type, $date,$numberatt_date) 
        {
...

        {
            ...
            $query = $this->db->query("select leave_types.type as `att_type`,leave_types.id as typid ,staff.name,staff.surname,staff.employee_id,staff.contact_no,staff.email,roles.name as user_type from staff  left join staff_roles on (staff.id = staff_roles.staff_id) left join roles on (roles.id = staff_roles.role_id) left join staff_leave_request on (staff.id = staff_leave_request.staff_id) 
left join leave_types on leave_types.id = staff_leave_request.leave_type_id   
  where staff_roles.role_id = roles.id and staff.is_active = 1 group by staff.id");         
...
$query = $this->db->query("select leave_types.type as `att_type`,leave_types.id as typid ,staff.name,staff.id,staff.surname,staff.employee_id,staff.contact_no,staff.email,roles.name as user_type from staff  left join staff_roles on (staff.id = staff_roles.staff_id) left join roles on (roles.id = staff_roles.role_id) left join staff_leave_request on (staff.id = staff_leave_request.staff_id) 
left join leave_types on leave_types.id = staff_leave_request.leave_type_id  where roles.name = '" . $user_type . "' and staff.is_active = 1 group by staff.id" );
    ...
    return $query->result_array();
     ...
     public function staff_leave_request() 
                {
                $query = $this->db->query("select * from staff_leave_request where status='approve'");
                return  $query->result_array(); 
                }
                ...
                {
                $query = $this->db->query("select * from staff where is_active='1'");
