        ...
        */
        ...
        $this->db->join("library","library.library_id=books.library","left");
        $this->db->join("librarysub","librarysub.lib_sub_id=books.subject","left");
        if ($id != null) {
        ...
        {
        $query=$this->db->get('librarysub');
        return $query->result_array();
        }
        
        public function getsubtypes() {
        $this->db->select('distinct(lib_sub_type)');
        $this->db->from('librarysub');
        $query = $this->db->get();
        return $query->result_array();
        }
        
        public function getsubs($subjecttype) {
        $this->db->select('*');
        $this->db->from('librarysub');
        $this->db->where('lib_sub_type',$subjecttype);
        $query = $this->db->get();
        return $query->result_array();
        }
        ...
        $this->datatables
        ->select('books.*,librarysub.*,library.*,IFNULL(total_issue, "0") as `total_issue` ')
        ->searchable('book_title,description,book_no,isbn_no,publish,author,lib_sub_name,library_name,rack_no,qty," ",perunitcost,postdate')
        ->orderable('book_title,description,book_no,isbn_no,publish,author,lib_sub_name,library_name,rack_no,qty," ",perunitcost,postdate')
        ...
        ->join('library','library.library_id=books.library','left')
        ->join('librarysub','librarysub.lib_sub_id=books.subject','left')
        ->from('books');
        ...
        */
        ...
        public function get_cprivileges()
        {
        $this->db->select('*');
        $this->db->from('library_circulationprivileges');
        $this->db->join('sessions', 'sessions.id = library_circulationprivileges.library_cp_session');
        $this->db->where('library_circulationprivileges.library_cp_session', $this->current_session);
        $query = $this->db->get();
        return $query->result_array();
        }
        ...
        public function get_privilegesbyid($id)
        {
        $this->db->select('*');
        $this->db->from('library_circulationprivileges');
        $this->db->join('sessions', 'sessions.id = library_circulationprivileges.library_cp_session');
        $this->db->where('library_circulationprivileges.library_cp_id', $id);
        $query = $this->db->get();
        return $query->row_array();
        }
        
        
        public function get_multifine_byprivilegeid($id)
        {
        $this->db->select('*');
        $this->db->from('library_multiplefine');
        $this->db->join('library_circulationprivileges', 'library_circulationprivileges.library_cp_id = library_multiplefine.lib_multi_privilege_id');
        $this->db->where(array('library_multiplefine.lib_multi_privilege_id'=> $id));
        $query = $this->db->get();
        return $query->result_array();
        }
        ...
        public function get_multifine()
        {
        $this->db->select('*');
        $this->db->from('library_multiplefine');
        $this->db->join('library_circulationprivileges', 'library_circulationprivileges.library_cp_id = library_multiplefine.lib_multi_privilege_id');
        $this->db->where(array('library_multiplefine.lib_multi_status'=> 1));
        $this->db->order_by('library_multiplefine.lib_multi_finedays','asc');
        $query = $this->db->get();
        return $query->result_array();
        }
