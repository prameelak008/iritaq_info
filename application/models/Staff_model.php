...
class Staff_model extends MY_Model
{
    ...
    public function staff_leave_request($current_date) 
    {
    $this->db->select('*');
    $this->db->from("staff");
    $this->db->join('staff_roles', "staff_roles.staff_id = staff.id");
    $this->db->join('staff_attendance', "staff_attendance.staff_id = staff.id");
    $this->db->where('date = ', $current_date);
    $this->db->where('staff_attendance_type_id !=', 3);
    $this->db->where('staff_attendance_type_id !=', 5);
...
public function staff_leave_request_holiday($current_date) 
{
$this->db->select('*');
$this->db->from("staff");
$this->db->join('staff_roles', "staff_roles.staff_id = staff.id");
...
$this->db->where('date = ', $current_date);
$this->db->where('staff_attendance_type_id =', 5);
$this->db->group_by('staff_roles.role_id');
$query = $this->db->get();
return $query->result_array();
}
  ...
  public function staff_leave_request_absent($current_date) 
    {
//$date = date('Y-m-d');
$this->db->select('*');
$this->db->from("staff");
...
return $query->result_array();
    ...
    $this->db->order_by('classes.serialno');
    $query = $this->db->get();
    return $query->result_array();
    }
    ...
    public function staff_category($category) 
    {

$query = $this->db->query("select * from student_session left join students  on (student_session.student_id = students.id) left join classes  on (student_session.class_id = classes.id) left join sections  on (student_session.section_id = sections.id) left join categories  on (students.category_id = categories.id ) where categories.id='" . $category . "'");
return $query->result_array();
}



/*dashboard view student_present*/
    public function student_leave_request($current_date) 
    {
$this->db->select('*');
...
public function student_leave_request_holiday($current_date) 
    {
...
return $query->result_array();
        ...
        public function student_leave_request_absent($current_date) 
        {
        $this->db->select('*');
        $this->db->from("students");
    ...
    public function batchInsert($data, $roles = array(), $leave_array = array(),$leavehrs_array = array(), $data_setting = array())
    {
        // $this->db->trans_start();
        // $this->db->trans_strict(false);
        $this->db->insert('staff', $data);
        ...
        // if (!empty($leavehrs_array)) {
        //     foreach ($leavehrs_array as $key => $value) {
        //         $leavehrs_array[$key]['staff_id'] = $staff_id;
        //     }

        //     $this->db->insert_batch('staff_leave_details', $leavehrs_array);
        // }
    ...
    */
    ...
    public function add_staff_leave_details($data2,$data3)
    {
        ...
        if (isset($data3['id'])) {
            ...
            $this->db->update('staff_leave_details', $data3);
        } else {
            $this->db->insert('staff_leave_details', $data3);
            return $this->db->insert_id();
        }
    }
 ...
 /*report ---------------------------Daily Attendence Report----------------------------------------------*/   
    
    
    public function staff_absent_request($current_date,$abstyp,$role) 
{
...
/*----------------------Report DailyAttendence----------------------------------------*/
public function student_absent_request($current_date,$abstyp,$class) 
{
...
return $query->result_array();
    ...
    return $query->result_array();
            ...
            public function getStaffPayroll_bymonth_val($id,$startMonth,$st_year_val)
            {
            $staff_payroll_val = array();
            $st=array();
            $st                = $st_year_val;
            ...
            if($array[$month]=="December")
            {
            $st++;
            }
            $this->db->select('*');
            $this->db->from('staff_payslip');
            $this->db->where(array('staff_id'=> $id,'month'=>$month,'year'=>$st));
            $query = $this->db->get();
            $staff_payroll_val[]= $query->result_array();
            }
            return $staff_payroll_val;
            }
            ...
            public function getStaffPayroll_previous($id,$startMonth,$st_year_val)
            {
            $staff_payroll_val = array();
            $st=array();
            $st                = $st_year_val;
    ...
    public function getLeaveDetails($id)
    {
        ...
        $query = $this->db->select('staff_leave_details.alloted_leave,staff_leave_details.alloted_leavehours,staff_leave_details.id as altid,leave_types.type,leave_types.id')->join("leave_types", "staff_leave_details.leave_type_id = leave_types.id", "inner")->where("staff_leave_details.staff_id", $id)->get("staff_leave_details");
    ...
    public function enablestaff($id)
    {
        ...
        $this->db->where("periodic_table_status", "1");
        $query = $this->db->get();
        return $query->result_array();
        }
    ...
    public function inventry_staff()
    {
     ...
     public function total_online_examination_currentsession()
    {

    $this->current_session = $this->setting_model->getCurrentSession();
    $this->db->select('*');
    $this->db->from('online_examination_accept');
    $this->db->where(array('online_examination_session_id='=>$this->current_session));
    return $this->db->count_all_results(); 
    }


     public function accepted_online_examination_currentsession()
    {

    $this->current_session = $this->setting_model->getCurrentSession();
    $this->db->select('*');
    $this->db->from('online_examination_accept');
    $this->db->where(array('online_examination_subject_status='=>2));
    ...
    return $this->db->count_all_results(); 
     ...
     public function rejected_online_examination_currentsession()
    {

    $this->current_session = $this->setting_model->getCurrentSession();
    $this->db->select('*');
    $this->db->from('online_examination_accept');
    $this->db->where(array('online_examination_subject_status'=>3));
    $this->db->where(array('online_examination_session_id='=>$this->current_session));
    return $this->db->count_all_results(); 
    }
