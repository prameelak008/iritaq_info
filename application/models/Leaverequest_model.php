    ...
    */
    ...
    public function staff_leave_request($id = null)
    {
     ...
     $this->current_session = $this->setting_model->getCurrentSession();
        ...
        $query = $this->db->select('staff.name,staff.surname,staff.employee_id,staff_leave_request.*,CONCAT(REPLACE(REPLACE(substring_index(leave_hours,":",2),":"," hrs "),"-","")," mins") as timedifference,leave_types.type')->join("staff", "staff.id = staff_leave_request.staff_id")->join("leave_types", "leave_types.id = staff_leave_request.leave_type_id")->where("staff.is_active", "1")->where("staff_leave_request.leave_session_id", $this->current_session) ->order_by("staff_leave_request.id", "desc")->get("staff_leave_request");
     ...
     public function staff_leave_request_previous($id = null)
    {
     ...
     $this->current_session = $this->setting_model->getCurrentSession();
        ...
        $query = $this->db->select('sessions.session,staff.name,staff.surname,staff.employee_id,staff_leave_request.*,CONCAT(REPLACE(REPLACE(substring_index(leave_hours,":",2),":"," hrs "),"-","")," mins") as timedifference,leave_types.type')->join("staff", "staff.id = staff_leave_request.staff_id")->join("sessions", "sessions.id = staff_leave_request.leave_session_id")->join("leave_types", "leave_types.id = staff_leave_request.leave_type_id")->where("staff.is_active", "1")->where("staff_leave_request.leave_session_id < ", $this->current_session) ->order_by("staff_leave_request.id", "desc")->get("staff_leave_request");
    ...
    public function user_leave_request($id = null,$current_session) 
    {
        ...
        */
         ...
         $query = $this->db->select('staff.name,staff.surname,staff.employee_id,staff_leave_request.*,CONCAT(REPLACE(REPLACE(substring_index(leave_hours,":",2),":"," hrs "),"-","")," mins") as timedifference,leave_types.type')->join("staff", "staff.id = staff_leave_request.staff_id")->join("leave_types", "leave_types.id = staff_leave_request.leave_type_id")->where("staff.is_active", "1")->where("staff.id", $id)->where("staff_leave_request.leave_session_id", $current_session)->order_by("staff_leave_request.id", "desc")->get("staff_leave_request");

        return $query->result_array();
    }
    
    
        public function user_leave_request_previous($id = null,$current_session) 
        {
        $query = $this->db->select('sessions.session,staff.name,staff.surname,staff.employee_id,staff_leave_request.*,CONCAT(REPLACE(REPLACE(substring_index(leave_hours,":",2),":"," hrs "),"-","")," mins") as timedifference,leave_types.type')->join("staff", "staff.id = staff_leave_request.staff_id")->join("leave_types", "leave_types.id = staff_leave_request.leave_type_id")->join("sessions", "sessions.id = staff_leave_request.leave_session_id")->where("staff.is_active", "1")->where("staff.id", $id)->where("staff_leave_request.leave_session_id<", $current_session)->order_by("staff_leave_request.id", "desc")->get("staff_leave_request");
    ...
    public function countLeavesData($staff_id, $leave_type_id,$current_session)
    {
        ...
        */
        ...
        $query1 = $this->db->select('SEC_TO_TIME( SUM(time_to_sec(leave_hours))) as approve_leave')->where(array('staff_id' => $staff_id, 'status!=' => 'disapprove', 'leave_type_id' => $leave_type_id,'leave_session_id'=>$current_session))->get("staff_leave_request");
        return $query1->row_array();
    }
