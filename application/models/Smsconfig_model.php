                    ...
                    public function get_whatsapp($id = null) 
                    {
                    
                    $this->db->select()->from('sms_config_wasup');
                    if ($id != null) {
                    $this->db->where('id', $id);
                    } else {
                    $this->db->order_by('id');
                    ...
                    public function add_wasup($data) {
                    $this->db->trans_start(); # Starting Transaction
                    $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
                    //=======================Code Start===========================
                    $this->db->where('type', $data['type']);
                    $q = $this->db->get('sms_config_wasup');
                    
                    if ($q->num_rows() > 0) {
                    $this->db->where('type', $data['type']);
                    $this->db->update('sms_config_wasup', $data);
                    $message      = UPDATE_RECORD_CONSTANT." On sms config id ".$data['type'];
                    $action       = "Update";
                    $record_id    = $data['type'];
                    $this->log($message, $record_id, $action);
                    ...
                    $this->db->insert('sms_config_wasup', $data);
                    $insert_id = $this->db->insert_id();
                    $message      = INSERT_RECORD_CONSTANT." On sms config id ".$insert_id;
                    $action       = "Insert";
                    $record_id    = $insert_id;
