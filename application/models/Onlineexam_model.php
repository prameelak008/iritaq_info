            ...
            public function get_subjectlist_old($getStaffID)
            {
               
                
            $this->db->select('*,class as classname,classes.class as classname,sections.section as sectionsname,students.email as email,students.gender as gender,students.current_address as current_address,students.firstname as firstname,students.middlename as middlename,students.lastname as lastname,students.admission_no as admission_no ,students.roll_no as roll_no,students.id as studid,students.mobileno as mobileno,online_examination.online_examination_examgroup as examgroup,online_examination.online_examination_exam as exambatch');

            $this->db->from('online_examination');
            $this->db->join('students', 'students.id  = online_examination.online_examination_student_id');
            $this->db->join('student_session', 'student_session.student_id = students.id');
            ...
            $this->db->where('class_teacher.staff_id', $getStaffID);
            $this->db->where('student_session.session_id',  $this->current_session);
            ...
            public function get_subjectlist($getStaffID)
            {              
            ...
            ///////get_subjectlist_by class
                    public function get_subjectlist_byclass($exam_group_id,$exam_id,$session_id,$class_id,$section_id,$getStaffID)
                    {            
                    $this->db->select('*,class as classname,classes.class as classname,sections.section as sectionsname,students.email as email,students.gender as gender,students.current_address as current_address,students.firstname as firstname,students.middlename as middlename,students.lastname as lastname,students.admission_no as admission_no ,students.roll_no as roll_no,students.id as studid,students.mobileno as mobileno,online_examination.online_examination_examgroup as examgroup,online_examination.online_examination_exam as exambatch');
...
////////
            ...
            public function get_class_by_teacher_old($getStaffID)
            {    
            $this->db->select('*,class as classname,classes.class as classname,sections.section as sectionsname,sessions.session as session');
            ...
            $this->db->join('sessions', 'sessions.id=class_teacher.session_id');
            $this->db->where('class_teacher.staff_id', $getStaffID);
            $query=$this->db->get();
            ...
            public function get_studentdetails($studid)
            {
            $this->db->select('*');
            $this->db->from('students');
            $this->db->join('student_session', 'student_session.student_id = students.id');
            $this->db->where('students.id', $studid);
            $query=$this->db->get();
            return $query->row_array();
            }
            ...
            public function get_subjectdetails($studid,$group,$batchid)
            {

            $this->db->select('*,exam_groups.name as groupname,exam_group_class_batch_exams.exam as examname,subjects.name as subject,subjects.code as subjectcode,
                online_examination.online_examination_student_id as studid,
            online_examination.online_examination_examgroup as groupid,online_examination.online_examination_exam as exambatchid');
            ...
            $this->db->where('online_examination.online_examination_examgroup', $group);
            $this->db->where('online_examination.online_examination_exam', $batchid);
            ...
            public function getadminsubjectdetails($students_array,$post_exam_id,$post_exam_group_id)
            {
                    ...
                    $subjects_details[] = $this->get_adminsubjectdetails($student_value,$post_exam_id,$post_exam_group_id);
            ...
            public function get_adminsubjectdetails($studid,$post_exam_id,$post_exam_group_id)
            {
            ...
            $this->db->where('online_examination.online_examination_student_id', $studid);
            $this->db->group_by('online_examination.online_examination_subject');
            ...
            public function get_subjectdetails_row($studid,$group,$batchid)
            {
            ...
            public function online_instruction()
            {
            $this->db->select('*');
            ...
            $this->db->where('online_examination_instruction.online_examination_current_session', $this->current_session);
            $query=$this->db->get();
            return $query->result_array();
            }
            ...
            public function edit_online_instruction($id)
            {
            $this->db->select('*');
            $this->db->from('online_examination_instruction');
            $this->db->join('exam_groups', 'exam_groups.id  = online_examination_instruction.online_examination_examgroup');
            $this->db->join('exam_group_class_batch_exams', 'exam_group_class_batch_exams.id  = online_examination_instruction.online_examination_examid');
            $this->db->where('online_examination_instruction.online_examination_id', $id);
            $query=$this->db->get();
            return $query->row_array();
            }
            ...
            public function others_instruction()
            {
            $this->db->select('*');
            $this->db->from('others_examination_instruction');

            $this->db->join('exam_groups', 'exam_groups.id  = others_examination_instruction.online_examination_examgroup');
            $this->db->join('exam_group_class_batch_exams', 'exam_group_class_batch_exams.id  = others_examination_instruction.online_examination_examid');


            $this->db->where('others_examination_instruction.online_examination_current_session', $this->current_session);
            $query=$this->db->get();
            return $query->result_array();
            }
            ...
            public function edit_others_instruction($id)
            {
            $this->db->select('*');
            $this->db->from('others_examination_instruction');
            $this->db->join('exam_groups', 'exam_groups.id  = others_examination_instruction.online_examination_examgroup');
            $this->db->join('exam_group_class_batch_exams', 'exam_group_class_batch_exams.id  = others_examination_instruction.online_examination_examid');
            $this->db->where('others_examination_instruction.online_examination_id', $id);
            $query=$this->db->get();
            return $query->row_array();
            }
            ...
            public function get_revaluation_list($getStaffID)
            {
                
            $this->db->select('*,student_session.class_id as class_id,student_session.section_id as section_id,student_session.session_id as session_id,classes.class as classname,sections.section as sectionsname,students.email as email,students.gender as gender,students.current_address as current_address,students.firstname as firstname,students.middlename as middlename,students.lastname as lastname,students.admission_no as admission_no ,students.roll_no as roll_no,students.id as studid,students.mobileno as mobileno,exam_group_exam_revaluation.exam_group_exam_revaluation_studentid as exam_group_exam_revaluation_studentid,
                exam_group_exam_revaluation.exam_group_exam_revaluation_examid as post_exam_id,exam_group_exam_revaluation.exam_group_exam_revaluation_examgroupid as post_exam_group_id');
            ...
            public function get_class_by_teacher($getStaffID)
            {    
            $this->db->select('*,class as classname,classes.class as classname,sections.section as sectionsname,sessions.session as session');
            $this->db->from('class_teacher');
            $this->db->join('classes', 'classes.id=class_teacher.class_id');
            ...
            return $query->row_array();
                ...
                public function get_attendence($students_array)
                {
                    ...
                    public function view_Studentattendence($students_array)
                    {
                    $attendence_details = array();
                    ...
                    public function getstudent_row($students_array)
                    {
                    $stud = array();
                    ...
                    {
                    $stud[] = $student_value;
                    $this->db->select('students.id as studdd');
                    $this->db->from('students');
                    $this->db->join('student_session', 'student_session.student_id  = students.id');
                    $this->db->where('students.id', $student_value);
                    $query=$this->db->get();
            ...
            {
            $this->db->select('students.*,student_session.id as session_id');
            $this->db->from('students');
            $this->db->join('student_session', 'student_session.student_id  = students.id');
            ...
            return $query->row_array();            
                        ...
                        public function online_examination_attendence_settings()
                        {
                        $this->db->select('*');
                        $this->db->from('online_examination_attendence_settings');
                        $this->db->join('exam_groups', 'exam_groups.id  = online_examination_attendence_settings.online_examination_examgroup');
                        $this->db->join('exam_group_class_batch_exams', 'exam_group_class_batch_exams.id  = online_examination_attendence_settings.online_examination_examid');
                        $this->db->where('online_examination_attendence_settings.online_examination_current_session', $this->current_session);
                        $query=$this->db->get();
                        return $query->result_array();
                        }
                        
                        
                        public function edit_online_attendence_settings($id)
                        {
                        $this->db->select('*');
                        $this->db->from('online_examination_attendence_settings');
                        $this->db->join('exam_groups', 'exam_groups.id  = online_examination_attendence_settings.online_examination_examgroup');
                        $this->db->join('exam_group_class_batch_exams', 'exam_group_class_batch_exams.id  = online_examination_attendence_settings.online_examination_examid');
                        $this->db->where('online_examination_attendence_settings.online_examination_id', $id);
                        $query=$this->db->get();
                        return $query->row_array();
                        }
                        }
