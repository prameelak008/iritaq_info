        ...
        */
        ...
        {
        $this->db->select('library_cp_id,library_circulationprivileges.library_cp_category,library_circulationprivileges.library_cp_no_of_days,book_issues.id,book_issues.return_date,book_issues.duereturn_date,book_issues.issue_date,book_issues.is_returned,book_issues.amount_paid,books.book_title,books.book_no,books.author,books.rack_no,books.isbn_no,books.perunitcost')->from('book_issues');
        $this->db->join('books', 'books.id = book_issues.book_id', 'left');
        $this->db->join('libarary_members', 'libarary_members.id = book_issues.member_id', 'left');
        $this->db->join('library_circulationprivileges', 'library_circulationprivileges.library_cp_category = libarary_members.member_type', 'left');
        
        if ($member_id != null) {
        $this->db->where('book_issues.member_id', $member_id);
        $this->db->order_by("book_issues.is_returned", "asc");
        }
        $this->db->group_by(['book_issues.book_id', 'book_issues.issue_date', 'book_issues.member_id']);
        $query = $this->db->get();
        return $query->result_array();
        }
        
        
        public function getFineDetails($cp_id) 
        {
        $this->db->select('*');
        $this->db->from('library_circulationprivileges');
        $this->db->join('library_multiplefine', 'library_multiplefine.	lib_multi_privilege_id = library_circulationprivileges.library_cp_id', 'left');
        $this->db->where(array('library_circulationprivileges.library_cp_status'=>1,'library_circulationprivileges.library_cp_session'=> $this->current_session,'library_circulationprivileges.library_cp_id'=>$cp_id));
        ...
        public function get_returndate($membertype)
        {
        $this->db->select('*');
        $this->db->from('library_circulationprivileges');
        $this->db->where(array('library_cp_status'=>1,'	library_cp_session'=> $this->current_session,'library_cp_category'=>$membertype));
        $query=$this->db->get();
        return $query->row_array();
        }
