...
class Schsettings extends Admin_Controller
{
    ...
    public function ajax_editlogo()
    {
        ...
        public function dashboard_editlogo()
        {

        $id=1;
        $this->db->where('dashboard_id',$id);
        $q = $this->db->get('sch_settings');
        if ( $q->num_rows() > 0 ) 
        {

        $config['upload_path']      = 'uploads/school_content/dashboardimage';
        ...
        $condition          = array('dashboard_id'     => $id);
        $data               = array('dashboard_logo'   => $pic);
    ...
    public function ajax_dashboardlogo()
    {
        $this->form_validation->set_rules('id', 'ID', 'trim|required|xss_clean');
        $this->form_validation->set_rules('file', 'Image', 'callback_handle_upload');
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (isset($_FILES["file"]) && !empty($_FILES['file']['name'])) {
                $fileInfo = pathinfo($_FILES["file"]["name"]);
                $img_name = $id . '.' . $fileInfo['extension'];
                move_uploaded_file($_FILES["file"]["tmp_name"], "./uploads/school_content/dashboardimage/" . $img_name);
            }

            $data_record = array('id' => $id, 'dashboard_logo' => $img_name);
