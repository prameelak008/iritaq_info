...
class Leaverequest extends Admin_Controller {
    ...
    function __construct() {
       ...
       function leaverequest()
       {
        ...
        $leave_request_previous         = $this->leaverequest_model->staff_leave_request_previous();
        $data["leave_request_previous"] = $leave_request_previous;
        ...
        {
            $count_leaves[] = $this->leaverequest_model->countLeavesData($id, $value["leave_type_id"],$current_session);
            $data[$i]['type'] = $value["type"];
            $data[$i]['id'] = $value["leave_type_id"];
           // $data[$i]['alloted_leave'] = $value["alloted_leave"];

            $data[$i]['alloted_leavehours'] = $value["alloted_leavehours"];
        ...
        */
         ...
         foreach ($data as $dkey => $dvalue) 
         {
            if (!empty($dvalue["alloted_leavehours"])) {
                if ($lid == $dvalue["id"]) {
                    $a = "selected";
                } else {
                    $a = "";
                ...
                if ($dvalue["alloted_leavehours"] == "") {

                    $available = $dvalue["approve_leave"];
                } else {
                    $available= $dvalue["alloted_leavehours"] - $dvalue["approve_leave"];
                }
                if ($available > 0) {
    ...
    function addLeave()
    {
        ...
        $this->form_validation->set_rules('leave_from_time', $this->lang->line('fromtime'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('leave_to_time', $this->lang->line('totime'), 'trim|required|xss_clean');
            ...
            if ($my_laeve['alloted_leave'] >= $leave_days) {
                ...
                if (!empty($request_id)) 
                {
                    ...
                    $data = array('id' => $request_id,
                        ...
                        'leave_to' => $leaveto,'leave_fromtime' => $leave_from_time,'leave_totime' => $leave_to_time, 'employee_remark' => $reason, 'status' => $status, 'admin_remark' => $remark, 'applied_by' => $applied_by, 'document_file' => $document,'leave_session_id'=>$current_session);
                } else {

                    $data = array('staff_id' => $staff_id, 'date' => date("Y-m-d", $this->customlib->datetostrtotime($applied_date)), 'leave_days' => $leave_days, 'leave_type_id' => $leavetype, 'leave_from' => $leavefrom, 'leave_to' => $leaveto,'leave_fromtime' => $leave_from_time,'leave_totime' => $leave_to_time, 'employee_remark' => $reason, 'status' => $status, 'admin_remark' => $remark, 'applied_by' => $applied_by, 'document_file' => $document,'leave_session_id'=>$current_session);
                }
...
$this->form_validation->set_rules('leave_to_time', $this->lang->line('totime'), 'trim|required|xss_clean');
    ...
    */
    ...
    function timeDifference($leavefrom,$leave_from_time,$leaveto,$leave_to_time) 
    {
        $dt1 = $leavefrom ." ".$leave_from_time;
        $dt2 = $leaveto." ".$leave_to_time;
        ...
        $this->form_validation->set_rules('userfile', $this->lang->line('file'), 'callback_handle_upload[userfile]');
            ...
            $leave_days = $this->dateDifference($leavefrom, $leaveto);
            $leave_hours = $this->timeDifference($leavefrom,$leave_from_time,$leaveto,$leave_to_time);
            $my_laeve = $this->leaverequest_model->myallotedLeaveType($staff_id, $leavetype);

            $my_lv = $this->leaverequest_model->countLeavesData($staff_id, $leavetype,$current_session);
            
            $leaveh = $this->timeDifference($leavefrom,$leave_from_time,$leaveto,$leave_to_time);
            $lvhrs=explode(":",$leaveh);
     
           $arr=explode(":",$my_lv['approve_leave']);
           $toarr=$arr[0]+$lvhrs[0];
          
            if ($my_laeve['alloted_leavehours'] >= $toarr)
            {
              
                //if ($my_laeve['alloted_leavehours'] >= $leave_hours)
                //{
                ...
                'leave_days' => $leave_days,
                'leave_from' => $leavefrom,
                'leave_to' => $leaveto, 'leave_hours' => $leave_hours,'employee_remark' => $reason, 'status' => $status, 'admin_remark' => $remark, 'applied_by' => $applied_by, 'document_file' => $document,'leave_session_id'=>$this->current_session);
                } 
                ...
                else
                 {
                    ...
                    $data = array('staff_id' => $staff_id, 'date' => date("Y-m-d", $this->customlib->datetostrtotime($applied_date)), 'leave_days' => $leave_days, 'leave_type_id' => $leavetype, 'leave_from' => $leavefrom, 'leave_to' => $leaveto, 'leave_fromtime' => $leave_from_time, 'leave_totime' => $leave_to_time, 'leave_hours' => $leave_hours, 'employee_remark' => $reason, 'status' => $status, 'admin_remark' => $remark, 'applied_by' => $applied_by, 'document_file' => $document,'leave_session_id'=>$this->current_session);
                }
