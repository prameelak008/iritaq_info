        ...
        class Sayexamte extends Admin_Controller
        {
        ...
        $this->session->set_userdata('sub_menu','say_exam/sayexam_te');
        ...
        $data['sayexam'] = $this->examresult_model->getsayexamte_subjectfor_teachers($exam_id, $exam_group_id,$class_id, $section_id,$session_id);
        ...
        $this->load->view('admin/sayexamte/index', $data);
        ...
        {
        $exam_group_exam_sayexam_id      = $this->input->post('exam_group_exam_sayexam_id');
        ...
        for($i=0;$i<count($exam_group_exam_sayexam_id);$i++)
        { 
        $data[$i]          =    array(                                 
        //'exam_group_exam_sayexam_updated_getcmarks'   =>  $get_cmarks[$i],
        'exam_group_exam_sayexam_updated_getmarks'    =>  $get_marks[$i],
        'exam_group_exam_sayexam_approvedstatus'      =>  2,
        );
        $this->db->where('exam_group_exam_sayexam_id', $exam_group_exam_sayexam_id[$i]);            
        $this->db->update('exam_group_exam_sayexam_temarks', $data[$i]);
        }
        ...
        public function  getstudent_sayexamtelist_modal ()
        {
        ...
        $data['sayexam'] = $this->examresult_model->getsayexamte_subjectfor_Marks($post_exam_id, $post_exam_group_id, $students_array);
        $data['getgroup_name'] = $this->examresult_model->getgroup_name($post_exam_id, $post_exam_group_id);
        $data['sayexam_approved_status'] = $this->examresult_model->sayexamte_approved_status($post_exam_id, $post_exam_group_id, $students_array);
        if (!$this->input->is_ajax_request())
        {
        exit('No direct script access allowed');
        }                             
        
        $this->load->view('admin/sayexamte/online_exam_ajax',$data);
        }
        ...
        $data['sayexam_marks']        = $this->examresult_model->admin_get_sayexamte_students($post_exam_id, $post_exam_group_id,$students_array);
        $data['sayexamfee_feecharge_te'] = $this->examgroup_model->sayexamfee_feecharge_temarks_admin($post_exam_id, $post_exam_group_id,$session_id);
        
        $data['sch_setting']             = $this->sch_setting_detail;
        $student_exam_page               = $this->load->view('admin/sayexamte/_printsayexam', $data, true); 
        ...
        echo json_encode($array); 
            ...
            $data                    = array();
            $this->session->set_userdata('top_menu', 'say_exam');
            $this->session->set_userdata('sub_menu', 'say_exam/sayexamte_payment');
            ...
            $data['online_examlist']  =  $this->examgroup_model->list_online_exam_sayexamte($exam_group_id,$exam_id,$session_id,$class_id,$section_id);
            $data['feelist']          =  $this->examgroup_model->online_exam_update_fees_sayexamte($exam_group_id,$exam_id,$session_id,$class_id,$section_id);
            
            }
            $data['current_session'] = $this->current_session; 
            $this->load->view('layout/header', $data);
            $this->load->view('admin/sayexamte/updatepayment', $data);
            $this->load->view('layout/footer', $data);
            }
            ...
            $year               = date('Y');
            $data                                           =  array(
            'fees_sayexampayment_orderid'                   =>  $orderid,
            'fees_sayexampayment_transaction_no'            =>  $transaction,
            'fees_sayexampayment_amount'                    =>  $amount,
            'fees_sayexampayment_transdate'                 =>  $transactiondate,
            'fees_sayexampayment_student_id'                =>  $student_id,
            'fees_sayexampayment_examgroup'                 =>  $exam,
            'fees_sayexampayment_examgroupbatch'            =>  $examgroup,
            'fees_sayexampayment_session_id'                =>  $session_id,
            'fees_sayexampayment_class_id'                  =>  $class_id,
            'fees_sayexampayment_section_id'                =>  $section_id,
            'fees_sayexampayment_statuscode'                =>  'S',
            'fees_sayexampayment_id_status'                 =>  '1',
            'fees_sayexampayment_updated_date'              =>  date('Y-m-d H:i:s'),
            'fees_sayexampayment_year'                      =>  date('Y'),
            'fees_sayexampayment_statuscode'                =>  'S',
            'fees_sayexampayment_errormsg'                  => 'Successfully processed by the office',
            'fees_sayexampayment_notes'                     =>  $notes
            );
            $query= $this->db->insert('fees_sayexampayment_temarks',$data);
            ...
            $cond=array('fees_sayexampayment_id'=>$id);
            $empdelete=$this->db->delete('fees_sayexampayment_temarks',$cond);
            if($empdelete)
            {
            echo true;
            } 
