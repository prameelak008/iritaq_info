...
if (!defined('BASEPATH'))
    ...
    */
        ...
        else
        {
            ...
            $search              = $this->input->post('search');
            $holiday             = $this->input->post('holiday');
            $monthlyleave        = $this->input->post('monthlyleave');
            $classleave          = $this->input->post('classleave');
            $specialleave        = $this->input->post('specialleave');
                ...
                foreach ($session_ary as $key => $value) {
                    ...
                    if ($checkForUpdate != 0) {
                        if (isset($holiday)) {
                            ...
                            else if (isset($monthlyleave)) {
                            $arr = array(
                                'id' => $checkForUpdate,
                                'student_session_id' => $value,
                                'attendence_type_id' => 7,
                                'remark' => $this->input->post("remark" . $value),
                                'date' => date('Y-m-d', $this->customlib->datetostrtotime($date))
                            );
                        }

                        else if (isset($classleave)) {
                            $arr = array(
                                'id' => $checkForUpdate,
                                'student_session_id' => $value,
                                'attendence_type_id' => 8,
                                'remark' => $this->input->post("remark" . $value),
                                'date' => date('Y-m-d', $this->customlib->datetostrtotime($date))
                            );
                        }
                        ...
                        else if (isset($specialleave)) {
                            $arr = array(
                                'id' => $checkForUpdate,
                                'student_session_id' => $value,
                                'attendence_type_id' => 9,
                                'remark' => $this->input->post("remark" . $value),
                                'date' => date('Y-m-d', $this->customlib->datetostrtotime($date))
                            );
                        }
    ...
    function attendencereport()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $attendencetypes = $this->attendencetype_model->get();
            $data['attendencetypeslist'] = $attendencetypes;
            $resultlist = $this->stuattendence_model->searchAttendenceClassSectionPrepareReport($class, $section, date('Y-m-d', $this->customlib->datetostrtotime($date)));
    ...
    function classattendencereport() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $studentlist = $this->student_model->searchByClassSectionRepo($class, $section);
    ...
    function monthAttendance($st_month, $no_of_months, $student_id) {
        ...
        return $record;
            ...
            public function sendSms_absent()
            {
