...
class Feemaster extends Admin_Controller {
    ...
    function index()
	{
        ...
        if ($this->form_validation->run() == FALSE) {
			...
			$this->db->where(array('id'=>$ftype[$i]));
			$query=$this->db->get();
			$mx= $query->row_array();
			
			$date = date_parse($mx['type']);
			$m = $date['month'];
	
			//$m=date('m',strtotime($mx['type']));

			
			$mm=$mx['type'];
			
			
			$duedate=$this->customlib->dateFormatToYYYYMMDD($this->input->post('due_date'));
			$d= date('d', strtotime($duedate));
			$y= date('Y', strtotime($duedate));
			$ts=strtotime($mm, $y);
			$t= date('t',$ts);
            ...
            $feegroup_result = $this->feesessiongroup_model->add($insert_array[$i]);
