...
class Smsconfig extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $sms_result              = $this->smsconfig_model->get();
         $whatsapp_result        = $this->smsconfig_model->get_whatsapp();
        $data['statuslist']      = $this->customlib->getStatus();
        $data['smslist']         = $sms_result;
        $data['whatsapp_result'] = $whatsapp_result;
    ...
    public function whatsapp()
        ...
        $this->form_validation->set_error_delimiters('', '');
        $this->form_validation->set_rules('whatsapp_link', $this->lang->line('whatsappformat'), 'required');
        $this->form_validation->set_rules('api', $this->lang->line('api_key'), 'required');
        $this->form_validation->set_rules('intense_id', $this->lang->line('intense_id'), 'required');
        $this->form_validation->set_rules('auth_token', $this->lang->line('auth_token'), 'required');
        $this->form_validation->set_rules('sender_phone_number', $this->lang->line('registered_phone_number'), 'required');
        ...
        if ($this->form_validation->run()) {
            ...
            $data = array(
                'type'      => 'WhatsApp',
                'url'       => $this->input->post('whatsapp_link'),
                'api_id'    => $this->input->post('api'),
                'senderid'  => $this->input->post('intense_id'),
                'authkey'   => $this->input->post('auth_token'),
                'contact'   => $this->input->post('sender_phone_number'),
                'is_active' => $this->input->post('status'),
            );
            $this->smsconfig_model->add_wasup($data);
            echo json_encode(array('st' => 0, 'msg' => $this->lang->line('update_message')));
            ...
            $data = array(
                ...
                'api'              => form_error('api'),
                'intense_id'         => form_error('intense_id'),
                'auth_token'          => form_error('auth_token'),
                'sender_phone_number' => form_error('sender_phone_number'),
