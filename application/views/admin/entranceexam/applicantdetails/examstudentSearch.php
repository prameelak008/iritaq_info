        ...
        <select autofocus="" id="admission_id" name="admission_id" class="form-control" >
        <option value=""><?php echo $this->lang->line('select'); ?></option>
        <?php
        $count = 0;
        foreach ($admissionlist as $adlist) {
        ?>
        <option value="<?php echo $adlist['admission_id'] ?>" <?php if (set_value('admission_id') == $adlist['admission_id']) {
        echo "selected=selected";
        }
        ?>><?php echo $adlist['admission_name'] ?></option>
        <?php
        $count++;
        }
        ?>
        ...
        <tr>
        <td><?php echo $sl;  ?></td>
        <td><?php echo $adlist['session'];  ?></td>
        <td style="font-size:15px;"><b > Registered Details</b><br>
         <?php echo $adlist['entrance_examgroup_name']; ?><br>
        <?php echo $adlist['entrance_reg_name']; ?><br>
        <?php echo $adlist['entrance_reg_phone']; ?><br>
        <?php echo $adlist['entrance_reg_email']; ?>
        </a></td>   
        
        <td>
        <?php
        if($adlist['admission_name']=="")
        {
        ?>
        <span style="color:#e57841;"> Not Applied For Any Exam</span>
        <?php     
        }
        else
        {
        ...
        ?>
        <a href="<?php echo base_url(); ?>EntranceExam/examview/<?php echo $adlist['admission_id'] ?>"> <?php echo $adlist['admission_name']; ?></a></td>
        <?php 
        }
        ?>
        
       <!-- <td><?php echo $adlist['admission_address'];  ?></td>-->
        <td><?php echo $adlist['admission_dob']; ?></td>
        <td><?php echo $adlist['admission_date']; ?></td>
        <td><?php echo $adlist['admission_application_no']; ?></td>
        <td><?php echo $adlist['admission_range']; ?></td>
        
        <td><?php echo $adlist['entranceexam_course_name']; ?></td>
         
         
        <td><?php echo $adlist['admission_institute_examcenter']; ?></td>
        
        <td>
            
            <?php 
            foreach($feelistbyadmission as $fee)
            {
            if($fee['fees_entrancepayment_student_id']==$adlist['admission_application_registerid'])
            {
            echo "Order".'&nbsp;'.$fee['fees_entrancepayment_orderid'];
            echo"<br>";
            
             echo "Payment Status".':'.$fee['fees_entrancepayment_statuscode']; 
             echo"<br>";
            }
            }
            ?>
        ...
        <td align="right">
        <a href="<?php echo base_url(); ?>EntranceExam/examview/<?php echo $adlist['admission_id']; ?>" class="btn btn-default btn-xs"  data-toggle="tooltip" title="<?php echo $this->lang->line('show'); ?>" >
        <i class="fa fa-reorder"></i>
        </a>
        
        
        <a href="<?php echo site_url(); ?>EntranceExam/deleteexam/<?php echo $adlist['admission_application_registerid']; ?>" class="btn btn-default btn-xs"  id="deleteexam"  data-toggle="tooltip" title="<?php echo $this->lang->line('delete'); ?>" >
        ...
        <a href="<?php echo base_url(); ?>EntranceExam/view/<?php echo $adlist['admission_id'] ?>">
        <img class="img-responsive img-thumbnail width150" alt="<?php echo $adlist['admission_name']?>" src="<?php echo base_url() . $image; ?>" alt="Image" style="width:600px;height:600px;"></a>
        </div>
        </div>
        </div>
        ...
        <h4>
        <a href="<?php echo base_url(); ?>EntranceExam/examview/<?php echo $adlist['admission_id'] ?>"> <?php echo $adlist['admission_name']; ?></a>
        </h4>
        ...
        <b style="font-size:15px; color:blue;"> Registered Details</b>
        <br>
        <b style="font-size:15px; "><?php echo $adlist['entrance_reg_name']; ?></b><br>
        <?php echo $adlist['entrance_reg_phone']; ?><br>
        <?php echo $adlist['entrance_reg_email']; ?>
        </a> 
        <br>
        ...
        <address>
        <strong><b><?php echo ('Address'); ?>: </b><?php echo $adlist['admission_address']  ?></strong><br>
        <b><?php echo ('Date Of Birth'); ?>: 
        
        <?php if ($adlist["admission_dob"] != null && $adlist["admission_dob"] != '0000-00-00') {echo date($this->customlib->getSchoolDateFormat(), $this->customlib->dateyyyymmddTodateformat($adlist['admission_dob']));}?></b><br>
        <b><?php echo ('Admission Date: '); ?><?php echo $adlist['admission_date'] ?><br>
        </address>
        </div>
        
        
        <div class="col-xs-6 col-md-6">
        <b><?php echo ('Application_No'); ?>:&nbsp;</b><?php echo $adlist['admission_application_no'] ?><br>
        <b><?php echo ('Register id'); ?>:&nbsp;</b><?php echo $adlist['admission_range'] ?><br>
        
        <b>Course Applied:&nbsp;</b><?php echo $adlist['admission_range'] ?><br>
        <b><?php echo ('Exam Center'); ?>:&nbsp;</b><?php echo $adlist['entranceexam_course_name'] ?><br>
        </div>
        ...
        <a href="<?php echo site_url(); ?>EntranceExam/deleteexam/<?php echo $adlist['admission_application_registerid'] ?>" class="btn btn-default btn-xs" id="deleteButton"  data-toggle="tooltip" title="<?php echo $this->lang->line('delete'); ?>" >
        <i class="fa fa-remove"></i>
        </a>
