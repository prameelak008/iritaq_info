    ...
    <?php } ?>
            ...
            if ($this->rbac->hasPrivilege('staff_payroll', 'can_add')) {
                ?>
                                                    ...
                                                    <?php } 
                                                        ...
                                                        <a href="javascript:void" onclick="getPayslipOld('<?php echo $staff['payslip_id']; ?>')"  role="button" class="btn btn-primary btn-xs checkbox-toggle edit_setting" data-toggle="tooltip" title="<?php echo $this->lang->line('Payslip View'); ?>" ><?php echo $this->lang->line('view'); ?> <?php echo $this->lang->line('payslip'); ?></a>
                                                        <?php
                                                    }
                                                  ?></td>
            ...
            if ($this->rbac->hasPrivilege('staff_payroll', 'can_add')) {
                ?>
                                                ...
                                                <a href="javascript:void" onclick="getPayslip('<?php echo $staff['payslip_id'] ?>')"  role="button" class="btn btn-primary btn-xs checkbox-toggle edit_setting" data-toggle="tooltip" title="">View Pre-payment Slip</a>
                                           <?php }
                                            ?></td>
...
<div id="payslipviewformattwo"  class="modal fade" role="dialog">
...
<div id="payslipviewformatone"  class="modal fade" role="dialog">

    <div class="modal-dialog modal-dialog2 modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h4 class="modal-title"><?php echo $this->lang->line('details'); ?>   <span id="print2"></span></h4>
            </div>
            <div class="modal-body" id="testdata1">
...
<script type="text/javascript">
    ...
    function getPayslip(id) 
    {
        ...
        var base_url = '<?php echo base_url() ?>';
        $.ajax({
            url: base_url + 'admin/payroll/payslipviewformattwo',
            type: 'POST',
            data: {payslipid: id},           
            success: function (result) 
            {
    ...
    function getPayslipOld(id) {
        var base_url = '<?php echo base_url() ?>';
        $.ajax({
            url: base_url + 'admin/payroll/payslipviewformatone',
            type: 'POST',
            data: {payslipid: id},           
            success: function (result) {
                $("#print2").html("<a href='#'  class='pull-right modal-title moprintblack'  onclick='printDataOld(" + id + ")'  title='Print' ><i class='fa fa-print'></i></a>");
                $("#testdata1").html(result);
            }
        });

        $('#payslipviewformatone').modal({
            show: true,
            backdrop: 'static',
            keyboard: false
        });
      ...
      function popup(data)
    {
        ...
        frameDoc.document.write('<link rel="stylesheet" href="' + base_url + 'backend/plugins/daterangepicker/daterangepicker-bs3.css">');
         ...
         frameDoc.document.write('<script src="https://code.jquery.com/jquery-3.5.1.js"></'+ 'script>');
