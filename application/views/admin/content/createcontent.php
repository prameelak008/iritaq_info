        ...
        <div class="col-md-12">
        <div class="form-group">
        <label for="exampleInputEmail1">Status</label></label><small class="req"> *</small>
        <br>
        <input type="radio" name="status"   value="1" checked="checked" >  View           
        <input type="radio" name="status" value="2">   Download
        </div>
        </div>
        </div>
        ...
        <th class="noExport"><?php echo $this->lang->line('action'); ?>
        </th>
        </tr>
        </thead>
        <tbody>
        ...
        $file_path = base_url('admin/content/download/' . $file);
        ...
        echo '<a href="' . $file_path . '" target="_blank">' . $file . '</a>';
        }
        }
        ?>
    ...
    <td>
        ...
        ?>
        <a data-placement="left" href="<?php echo base_url(); ?>admin/content/delete/<?php echo $data['id'] ?>"class="btn btn-default btn-xs"  data-toggle="tooltip" title="<?php echo $this->lang->line('delete'); ?>" onclick="return doconfirm();" >
        ...
        <h5 class="modal-title" id="imageZoomLabel">View</h5>
        ...
        <img id="zoomedImage" src="" alt="Preview" class="img-fluid rounded" style="max-width: 100%; max-height: 80vh;" />
        </div>
        </div>
        </div>
        </div>
...
document.getElementById('form1').addEventListener('submit', function (e) {
    ...
    if (status === "1") {
        ...
        if (!allowed.includes(fileExt)) {
            alert("Only JPG, JPEG, or PNG files are allowed for 'View' status.");
