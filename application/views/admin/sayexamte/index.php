        ...
        <h1>
        <i class="fa fa-map-o"></i> <?php echo $this->lang->line('examinations'); ?> <small><?php echo $this->lang->line('student_fee'); ?></small>  </h1>
        </section>
        <!-- Main content -->
        ...
        <span style="color:#293174; background-color:#f1f1f1;">Application For SayExam-TE</span></h3>
        
        </div>
        
        <div class="box-body">
        <form role="form" action="<?php echo site_url('user/sayexam_temarks/sayexam') ?>" method="post" class="row">
        ...
        <button type="submit"  onclick="return checkvalue();" name="search" value="search_filter" class="btn btn-primary pull-right btn-sm checkbox-toggle"><i class="fa fa-search"></i> <?php echo $this->lang->line('search'); ?></button>
            ...
            <?php
            // $transDateStr = '20-04-2025 12:35:01'; // Format: dd-mm-yyyy HH:MM:SS
            
            // // Convert the string to a DateTime object
            // $transDate = DateTime::createFromFormat('d-m-Y H:i:s', $transDateStr);
            
            
            // // Get timestamp of the transaction
            // $transTimestamp = $transDate->getTimestamp();
            ...
            // if (($currentTimestamp - $transTimestamp) >= $thirtyDaysInSeconds) {
            // echo "Open application – Testing In Progress.";
            // $Chances = true;
            // } else {
            // echo "Still within 30 days – application closed.";
            // }


        

          if(empty($sayexamfee_payment ))
          {
        
        // if(empty($sayexamfee_payment ) || ($Chances))
        // {
        if (empty($marksheet)) 
        {
        ?>
        ...
        {
        $sayexam_subject[]=$gt['exam_group_exam_sayexam_subject_id'];
        }
        ...
        <form name="" method="POST" action="<?php   echo site_url('user/sayexam_temarks/send_request');?>">
        ...
        <span style="font-weight:bold;"></b>Apply Subject For Say Exam :<b></span>
        <br>
        <br>
        ...
        <tr>
        <th rowspan="2" class="headerclass" >
        ...
        <input type="hidden" value="<?php echo $exam_result_value->exam_group_exam_results_id ; ?>"/>
        ...
        */
        ...
        <?php
        echo $sayexamfee_feecharge_te['examfees_charge_fees_charge'].''.'/Paper';
        ?>
        </td>
        <td>
        </td>
        ...
        <?php
        echo $sayexamfee_feecharge_te['examfees_charge_processing_charge'];
        ?>
        ...
        <?php
        if($sayexamfee_feecharge_te['examfees_charge_fine']!="")
        {
        echo $sayexamfee_feecharge_te['examfees_charge_fine'];
        }
        else
        {
        echo "0";
        ...
        <?php
        if($sayexamfee_feecharge_te['examfees_charge_effectivedate']!="")
        {
        echo $sayexamfee_feecharge_te['examfees_charge_effectivedate'];
        }
        else
        {
        echo "00:00:0000";
        ...
        /*
        if($sayexam_approval['exam_group_exam_sayexam_approvedstatus']< 2 )
        {
        ?>
        <table style="height:50%;">                        
        <tr><td><input type="submit" name="submit" value="Apply For SayExam" onclick="return doconfirm();" class="btn btn-primary pull-right btn-sm checkbox-toggle"/></td></tr>
        </table>
        <?php
        }
        elseif($sayexam_approval['exam_group_exam_sayexam_approvedstatus']==2)
        {
        ?>
        <table style="height:50%;">                        
        ...
        */
        ?>
        <input type="submit" name="submit" value="Apply For SayExam" onclick="return doconfirm();" class="btn btn-primary pull-right btn-sm checkbox-toggle"/>
        </form>
        </div>
        ...
        <div style="width:100%">
        <span>
        <h5 style="text-align:center">SAY EXAM STATUS-TE </h5>
        </div>
        
        <form method="post" action="<?php echo base_url('user/sayexam_temarks/printsayexam') ?>" id="printMarksheet">
        ...
        $feetotal=0;
        $feegrandtotal=0;
        $totalfee=$sayexamfee_feecharge_te['examfees_charge_fees_charge'];
        
        // $totalfee="500";
        
        $totalamt="0";
        $processingcharge=$sayexamfee_feecharge_te['examfees_charge_processing_charge'];
        ...
        <td>
        <?php echo number_format($totalfee,2);  ?></td>
        </tr>
        ...
        <td colspan="2"><b><?php   $tot=$totalamt+$processingcharge;
        ...
        <input type="button" name="submit" value="Already Paid" class="btn btn-warning pull-right btn-sm checkbox-toggle"/>
        <input type="submit" name="submit" value="Print TE Say Exam" style="margin-right:20px" class="btn btn-success pull-right btn-sm checkbox-toggle"/>
        </form>
        ...
        function doconfirm()
        {
        job=confirm("Are you sure you want to continue your  TE exam application?");
        if(job!=true)
        {
        return false;
        }
        ...
        dataType:"JSON",
        url: "<?php echo site_url('user/sayexam_temarks/getstudentbatch_id');?>",
