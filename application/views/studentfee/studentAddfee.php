...
<br>
    ...
    <form action="<?php echo site_url('Printpdf/printFeesBy_Newpdf'); ?>"  method="POST" >
    <input type="submit" name="Print" value="Print Pdf" class="btn btn-sm btn-success"  >
    <input type="submit" name="sendmail" value="Send Mail" class="btn btn-sm btn-success"  >
...
if ($feetype_balance > 0 && strtotime($fee_value->due_date) < strtotime(date('Y-m-d'))) {
        ...
        ?>
                                                ...
                                                <input class="checkbox" type="checkbox" id="fee_checkbox" name="fee_checkbox[]" data-fee_master_id="<?php echo $fee_value->id; ?>" data-fee_session_group_id="<?php echo $fee_value->fee_session_group_id; ?>" data-fee_groups_feetype_id="<?php echo $fee_value->fee_groups_feetype_id; ?>" value="<?php echo $fee_value->fee_session_group_id; ?>,<?php echo $fee_value->id; ?>,<?php echo $fee_value->fee_groups_feetype_id; ?>">
                                                            ...
                                                            <div class="btn-group ">
                                                               <div class="pull-right">
                                                                <?php if ($this->rbac->hasPrivilege('collect_fees', 'can_delete')) {?>
                                                                    <button type="button" class="btn btn-default btn-xs" data-invoiceno="<?php echo $fee_value->student_fees_deposite_id . "/" . $fee_deposits_value->inv_no; ?>" data-main_invoice="<?php echo $fee_value->student_fees_deposite_id ?>" data-sub_invoice="<?php echo $fee_deposits_value->inv_no ?>" data-toggle="modal" data-target="#confirm-delete" title="<?php echo $this->lang->line('revert'); ?>">
                                                                        <i class="fa fa-undo"> </i>
                                                                    </button>
                                                                <?php }?>
                                                                <button type="button"  class="btn btn-xs btn-default printDoc" data-main_invoice="<?php echo $fee_value->student_fees_deposite_id ?>" data-sub_invoice="<?php echo $fee_deposits_value->inv_no ?>"  title="<?php echo $this->lang->line('print'); ?>"><i class="fa fa-print"></i> </button>
                                                            </div>
                                                          </div>
                                                        </td>
                                                    </tr>
...
if ($discount_value['status'] == "applied") {
            ?>
                                                            ...
                                                            <?php if ($this->rbac->hasPrivilege('collect_fees', 'can_delete')) {?>
                                                                <button type="button"  class="btn btn-default btn-xs" data-discounttitle="<?php echo $discount_value['code']; ?>" data-discountid="<?php echo $discount_value['id']; ?>" data-toggle="modal" data-target="#confirm-discountdelete" title="<?php echo $this->lang->line('revert'); ?>">
                                                                    <i class="fa fa-undo"> </i>
                                                                </button>
                                                                <?php
}
...
?></td>  <td class="text text-right"></td>
     ...
     <link rel="stylesheet" href="<?php echo base_url();?>assets_lib/alertify_new/css/alertify.css">                               
                            <script src="<?php echo base_url();?>assets_lib/alertify_new/alertify.js "></script>
                            <script type="text/javascript">
                                $(document).ready(function () {
                            <?php
                            if ($this->session->flashdata('flashError')) {
                                ?>
                             alertify.error("<?php echo $this->session->flashdata('flashError'); ?> ");
                                <?php
                            }
                            if ($this->session->flashdata('flashSuccess')) {
                                ?>
                                        alertify.success("<?php echo $this->session->flashdata('flashSuccess'); ?> ");
                                <?php
                            }
                            ?>
                                });
...
<div id="listCollectionModal" class="modal fade">
    <div class="modal-dialog">
        <form action="<?php echo site_url('studentfee/addfeegrp'); ?>"  method="POST" id="collect_fee_group" name="collect_fee_group">
            <div class="modal-content">
<!-- //================ -->
...
<script type="text/javascript">
        ...
        $(document).on('click', '.printSelected', function () 
		{
            ...
            var array_to_print = [];
            $.each($("input[id='fee_checkbox']:checked"), function () {
                var fee_session_group_id = $(this).data('fee_session_group_id');
                var fee_master_id = $(this).data('fee_master_id');
                var fee_groups_feetype_id = $(this).data('fee_groups_feetype_id');
                item = {};
        ...
        $(document).on('click', '.printpdf', function () 
        {
            ...
            if (array_to_print.length === 0) {
                ...
                $.ajax({
                    url: '<?php echo site_url("Printpdf/printFeesBy_Newpdf") ?>',
                    type: 'post',
