...
class TransactionRequestBean extends RequestValidate
{
    ...
    public function getEncryptedData()
    {
        try {
            ...
            if (!$this->isBlankOrNull($this->student_id)) {
            $clientMetaData .= "{studentid:" . $this->student_id . "}";
            }
            
            if (!$this->isBlankOrNull($this->examgroup)) {
            $clientMetaData .= "{examgroup:" . $this->examgroup . "}";
            }
            
            if (!$this->isBlankOrNull($this->examgroupbatch)) {
            $clientMetaData .= "{examgroupbatch:" . $this->examgroupbatch . "}";
            }
            
            if (!$this->isBlankOrNull($this->class_id)) {
            $clientMetaData .= "{class_id:" . $this->class_id . "}";
            }
            
            if (!$this->isBlankOrNull($this->section_id)) {
            $clientMetaData .= "{section_id:" . $this->section_id . "}";
            }
            
            if (!$this->isBlankOrNull($this->session_id)) {
            $clientMetaData .= "{session_id:" . $this->session_id . "}";
            }
            ...
            if (($this->requestType == "TWD") || ($this->requestType == "TRS") || ($this->requestType == "TWC") || ($this->requestType == "TRC") || ($this->requestType == "TIC") || ($this->requestType == "TICR")) {
                ...
                $aes = new AESPAY($cardInfoBuff, $this->key, $this->blockSize, $this->mode, $this->iv);
                $aes->require_pkcs5();
                $cardInfoStr = $aes->encryptHex();

                $aesObj = new AESPAY($cardInfoStr, $this->key, $this->blockSize, $this->mode, $this->iv);
                ...
                $aes = new AESPAY($impsInfoBuff, $this->key, $this->blockSize, $this->mode, $this->iv);
                $aes->require_pkcs5();
                $impsInfoStr = $aes->encryptHex();

                $aesObj = new AESPAY($impsInfoStr, $this->key, $this->blockSize, $this->mode, $this->iv);
                $aesObj->require_pkcs5();
            ...
            $aesObj = new AESPAY($this->strReqst, $this->key, $this->blockSize, $this->mode, $this->iv);
            $aesObj->require_pkcs5();
