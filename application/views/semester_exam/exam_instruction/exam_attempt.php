            ...
            */
            ...
            <th style="width: 150px;">Valid Year(upto)</th>
            ...
            <td>
            <input type="number"
            name="limits[<?php echo $key; ?>]"
            min="1"
            class="form-control <?php echo ($exam === 'Regular') ? 'readonly-field' : ''; ?>"
            <?php echo ($exam === 'Regular') ? 'readonly title="Attempts for Regular exam are fixed"' : 'required'; ?>
            value="<?php echo ($exam === 'Regular') ? '1' : ''; ?>">
            ...
            <td>
            <input type="number" 
            name="valid_year[<?php echo $key; ?>]" 
            min="0" 
            class="form-control <?php echo ($exam === 'Regular') ? 'readonly-field' : ''; ?>" 
            required
            <?php echo ($exam === 'Regular') ? 'readonly title="Valid years for Regular exams are fixed"' : ''; ?>
            value="<?php echo ($exam === 'Regular') ? '1' : ''; ?>">
            <?php if ($exam === 'Regular'): ?>
            <small class="text-muted" style="display: block; margin-top: 5px;">Default (always 1)</small>
            <?php endif; ?>
            </td>              
            </tr>
            <?php endforeach; ?>
            ...
            💡 Tip: Total number of times a student can take the exam.
            </div>
            </div>
            ...
            <span>Regular: <strong id="default_regular">1</strong></span>
            ...
            <div class="form-group"> <label><?php echo $this->lang->line('batch'); ?>
            <small class="req">*</small></label> 
            <select id="sem_type_students" name="sem_type" class="form-control sem_type"> 
            <option value="">-- Select Batch & Semester --</option>
            </select> <span class="text-danger"><?= form_error('sem_type'); ?></span> 
            </div>
            ...
            <div class="modal fade" id="attemptEditModal" tabindex="-1" role="dialog">
            <div class="modal-dialog" role="document">
            <div class="modal-content">
            <div class="modal-header">
            <h5 class="modal-title">Update Attempt: <span id="modal_subject_name"></span></h5>
            </div>
            <form id="editAttemptForm">
            <div class="modal-body">
            <p>Student: <strong id="modal_student_name"></strong></p>
            <p>Exam Type: <span class="badge badge-info" id="modal_exam_type_label"></span></p>
            ...
            <input type="hidden" name="sem_group_id" id="hidden_sem_group_id">

            <div class="form-group">
            <label>Attempt Number</label>
            <input type="number" class="form-control" name="attempt_count" id="modal_attempt_count" min="0">
            </div>
            ...
            <button type="button" class="btn btn-xs btn-default border" data-dismiss="modal">Close</button>
            <button type="submit" class="btn btn-xs btn-primary" onclick="saveAttemptUpdate()">Save Changes</button>
            </div>
            </form>
            </div>
            </div>
...
$(document).ready(function() {
    ...
    // Load limits when batch changes
    $('#sem_type').on('change', function() {
        
        var sem_type_id = $(this).val();
        ...
        $.ajax({
            url: '<?php echo site_url('semester_exam/exam_attempt/get_existing_limits'); ?>',
            type: 'POST',
            dataType: 'json',
            data: { 
            ...
            success: function(response) {
                console.log(response); // Check browser console for debug
                
                if (response.success && response.data) {
                    // Fill the form
                    $.each(response.data, function(i, limit) {
                        $('input[name="limits[' + limit.c_exam_type + ']"]').val(limit.c_max_attempts);
                        $('input[name="valid_year[' + limit.c_exam_type + ']"]').val(limit.c_valid_years);
                    });
                    alert('✅ Existing limits loaded!');
                } else {
                    clearFields();
                    alert('📝 No limits found. Please set new limits.');
                }
            },
            error: function(xhr, status, error) {
                console.log(xhr.responseText); // Check browser console for error
...
function clearFields() {
    $('input[name^="limits["]').not('[readonly]').val('');
    $('input[name^="valid_year["]').not('[readonly]').val('');
}
            ...
            <div class="attempt-label">📝 Regular(Default-1)</div>
            <input type="number" 
            ...
            class="form-control attempt-input" 
            placeholder="More than 1 attempts" 
            min="1">
            </div>
            <div class="attempt-item">
            ...
            //     $('#studentEmptyState').html('<div class="loading-spinner"></div><p>Fetching student list...</p>').show();
            ...
            //         type: 'POST',
            //         dataType: 'json',
            //         data: { sem_type: sem_type },
            //         success: function(response) {
            //             if (response && response.length > 0) {
            //                 displayStudentsCollapsible(response, sem_type);
            //             } else {
            //                 $('#studentEmptyState').html('<h3>No Students Found</h3>').show();
            //             }
            //         }
            //     });
            // }
            ...
            $('#studentEmptyState').html('<p>⏳ Loading Student Records...</p>').show();
            $('#studentDetailsSection').hide();

            // 1. Fetch Subjects
            $.ajax({
            url: '<?php echo site_url("semester_exam/exam_attempt/get_attempt_subjects_ajax"); ?>',
            type: 'POST',
            dataType: 'json',
            data: { sem_type: sem_type },
            ...
            {
            renderStudentSubjectTable(students, subjects);
            } 
            else {
            $('#studentEmptyState').html('<p>No students found.</p>').show();
            }
            }
            });
            }
            ...
            function renderStudentSubjectTable(students, subjects) {
            ...
            <th width="75%">Subject Details (Attempts Management)</th>
            </tr>
            </thead>
            <tbody>`;
            ...
            subjectHtml += `
            <div class="subject-row" style="margin-bottom: 10px; border: 1px solid #ccc; border-radius: 5px; overflow: hidden;">
            <div class="collapsible-header" onclick="toggleSubject(this)" style="padding: 12px; background: #f8f9fa; cursor: pointer; display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #eee;">
            <span><i class="fa fa-book"></i> <strong>${sub.name}</strong> <small class="text-muted">(${sub.code || sub.subject_code})</small></span>
            <span class="badge badge-info">View Attempts ▼</span>
            </div>
            <div class="collapsible-body" style="display: none; padding: 15px; background: #fff;">
            ...
            <tbody>
            ${['Regular', 'Supplementary', 'Revaluation', 'Improvement'].map(type => {
            // Set default rules for allowed attempts
            let allowed = (type === 'Regular') ? 1 : 3; 
            let used = 0; // This should ideally come from your student data object
            let remaining = allowed - used;
            let hideEdit = (type === 'Regular') ? 'style="display:none;"' : '';

            return `

            <tr>
            <td class="text-left"><strong>${type}</strong></td>
            <td id="allow_${student.id}_${sub.id}_${type}">${allowed}</td>
            <td id="used_${student.id}_${sub.id}_${type}" class="text-primary font-weight-bold">${used}</td>
            <td id="rem_${student.id}_${sub.id}_${type}" class="${remaining <= 0 ? 'text-danger' : 'text-success'}">
            ${remaining}
            </td>
            <td>
            <button class="btn btn-xs btn-default border"  ${hideEdit}
            onclick="openEditModal('${student.id}', '${sub.id}', '${student.firstname}', '${sub.name}', '${type}', ${allowed}, ${used})">
            ...
            <strong class="text-primary">${student.firstname}</strong><br>
            <small class="text-muted">Roll: <strong>${student.roll_no}</strong></small>
            </div>
            </td>
            <td>${subjectHtml}</td>
            </tr>`;
            ...
            html += `</tbody></table>`;
            $('#studentInfoBox').html(html);
            }



            // Helper to toggle the collapse
            function toggleSubject(header) {
            $(header).next('.collapsible-body').slideToggle(200);
            }

            // Function to open the Edit Modal
            function openEditModal(studentId, subjectId, studentName, subjectName, type) {
            $('#modal_student_name').text(studentName);
            $('#modal_subject_name').text(subjectName);
            $('#modal_exam_type_label').text(type);

            $('#hidden_student_id').val(studentId);
            $('#hidden_subject_id').val(subjectId);
            $('#hidden_exam_type').val(type);
            $('#hidden_sem_group_id').val($('#sem_type_students').val()); // get from filter

            $('#attemptEditModal').modal('show');
            }
            ...
            function saveAttemptUpdate() 
            {
            var formData = $('#editAttemptForm').serialize(); // Grabs all inputs automatically

            $.ajax({
            url: '<?php echo site_url("semester_exam/exam_attempt/save_student_attempt"); ?>',
            type: 'POST',
            data: formData,
            dataType: 'json',
            ...
            alert(res.message);
            $('#attemptEditModal').modal('hide');
            loadStudentData(); // Refresh the list to show new counts
            } else {
            alert(res.message);
            }
            }
